...
int main(int argc, char*argv[])
{
  for (;;)
  {
    ...
    if (json_as_variant_object.contains("trx"))
    {
      ...
      fbtc::client::trx_message trx_msg;
      try
      {
        trx_msg = json_as_variant.as<fbtc::client::trx_message>();
      }
      catch (const fc::exception& e)
      {
        elog("Error interpreting json as fbtc::client::trx_message: ${what}", ("what", e.to_string()));
      }

      ilog("Parsed fbtc::client::trx_message with id ${msg_id} containing transaction ${trx_id}",
        ("msg_id", fbtc::net::message(trx_msg).id())("trx_id", trx_msg.trx.id()));
    }
  }
