...
int main()
{
    ...
    cout << "Welcome to the Fbtc Web Update Utility. This tool is not particularly well-written. This tool is not user friendly. It is not for users. It is for developers. Deal with it.\n\nWould you like to (p)repare a new update, or (s)ign an existing one? ";
    cin >> option;
    ...
    if (option == 'p')
    {
        ...
        if (tolower(option) == 'y')
        {
            ...
            getline(cin, wif);
            auto key = fbtc::utilities::wif_to_key(wif);
            while (!key)
            {
                cout << "Couldn't parse that key. Enter WIF key: ";
                getline(cin, wif);
                key = fbtc::utilities::wif_to_key(wif);
            }
