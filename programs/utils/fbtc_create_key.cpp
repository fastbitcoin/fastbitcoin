...
int run( int64_t index, fc::optional<string> seed, fc::optional<fc::path> json_outfile )
{
    ...
    if( !json_outfile.valid() )
    {
        ...
        obj["wif_private_key"] = fbtc::utilities::key_to_wif(key);
        obj["native_address"] = fbtc::blockchain::address(key.get_public_key());
        obj["pts_address"] = fbtc::blockchain::pts_address(key.get_public_key());
    ...
    else
    {
        ...
        std::cout << "fbtc address: "
        << std::string(fbtc::blockchain::address(key.get_public_key())) << "\n";
