...
namespace fbtc { namespace blockchain {
   ...
   const uint8_t withdraw_p2sh_multisig::type	  = withdraw_p2sh_multisig_type;
   ...
   {
	   if (type == withdraw_multisig_type || type == withdraw_p2sh_multisig_type)
	   {
			return address(*this, address_type::multisig_address);
	   }
	   return address(*this);
   }
   ...
   set<address> withdraw_condition::owners()const
   {
       switch( withdraw_condition_types( type ) )
       {
		   ...
		   {
			   return set<address>{this->as<withdraw_p2sh_multisig>().owner};
		   }
           default:
               return set<address>();
       }
   ...
   optional<address> withdraw_condition::owner()const
   {
       switch( withdraw_condition_types( type ) )
       {
		   ...
		   case withdraw_p2sh_multisig_type:
			   return this->as<withdraw_p2sh_multisig>().owner;
           default:
                                                              ...
                                                              bool use_stealth_address)
   {
      ...
      if( memo_message.size() <= FBTC_BLOCKCHAIN_MAX_MEMO_SIZE )
      {
...
namespace fc {
   void to_variant( const fbtc::blockchain::withdraw_condition& var,  variant& vo )
   {
      ...
      switch( (withdraw_condition_types) var.type )
      {
         ...
         case withdraw_escrow_type:
            ...
            break;
		 case withdraw_p2sh_multisig_type:
			 obj["data"] = fc::raw::unpack<withdraw_p2sh_multisig>(var.data);
			 break;
   ...
   void from_variant( const variant& var,  fbtc::blockchain::withdraw_condition& vo )
   {
      ...
      switch( (withdraw_condition_types) vo.type )
      {
         ...
         case withdraw_escrow_type:
            ...
            return;
		 case withdraw_p2sh_multisig_type:
			 vo.data = fc::raw::pack(obj["data"].as<withdraw_p2sh_multisig>());
			 return;
         // No default to force compiler warning
      }
   ...
   { try {
      ...
      if( obj.contains( "from" ) )
         vo.from = obj["from"].as<fbtc::blockchain::public_key_type>();
      ...
      if( obj.contains( "memo_flags") )
         vo.memo_flags = obj["memo_flags"].as<fbtc::blockchain::memo_flags_enum>();
