...
catch( const fbtc::blockchain::price_multiplication_undefined& )
{
    return price();
}
catch( const fbtc::blockchain::price_multiplication_overflow& )
{
    return price();
}
catch( const fbtc::blockchain::price_multiplication_underflow& )
{
    return price();
}
...
asset market_order::get_quantity( const price& relative )const
{
  switch( order_type_enum( type ) )
  {
     ...
     case cover_order:
     {
        return asset( (*collateral * FBTC_BLOCKCHAIN_MCALL_D2C_NUMERATOR) / FBTC_BLOCKCHAIN_MCALL_D2C_DENOMINATOR );
     }
     default:
