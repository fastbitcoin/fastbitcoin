...
#include <fbtc/blockchain/market_engine_v7.hpp>
...
namespace fbtc { namespace blockchain { namespace detail {
  ...
  bool market_engine_v7::execute( asset_id_type quote_id, asset_id_type base_id, const fc::time_point_sec timestamp )
  {
      try
      {
          ...
          if( _pending_state->get_head_block_num() >= FBTC_V0_6_0_FORK_BLOCK_NUM )
          {
              FC_ASSERT( !_pending_state->is_fraudulent_asset( *quote_asset ) );
              FC_ASSERT( !_pending_state->is_fraudulent_asset( *base_asset ) );
          }

          if( _pending_state->get_head_block_num() >= FBTC_V0_9_0_FORK_BLOCK_NUM )
          {
          ...
          if( _pending_state->get_head_block_num() >= FBTC_V0_5_0_FORK_BLOCK_NUM )
          {
              ...
              if( _pending_state->get_head_block_num() >= FBTC_V0_8_0_FORK_BLOCK_NUM && _feed_price.valid() )
              {
  ...
  { try {
      ...
      {
          // Partial cover
          if( _pending_state->get_head_block_num() >= FBTC_V0_7_0_FORK_BLOCK_NUM )
              interest_paid = get_interest_paid_fixed( mtrx.ask_received, _current_collat_record.interest_rate, cover_age );
          else if( _pending_state->get_head_block_num() >= FBTC_V0_4_23_FORK_BLOCK_NUM )
  ...
  {
      if( _pending_state->get_head_block_num() >= FBTC_V0_8_0_FORK_BLOCK_NUM )
          return get_next_bid_v065();
      else if( _pending_state->get_head_block_num() >= FBTC_V0_7_0_FORK_BLOCK_NUM )
      ...
      */
      while( _current_bid && _collateral_itr.valid() )
      {
            ...
            cover_ask.get_price().base_asset_id == _base_id )
        {
            ...
            if( _pending_state->get_head_block_num() >= FBTC_V0_4_24_FORK_BLOCK_NUM )
            {
                --_collateral_itr;
                continue;
            }
...
void market_engine_v7::update_market_history( const asset& base_volume,
  ...
  asset market_engine_v7::get_interest_paid_fixed(const asset& total_amount_paid, const price& original_apr, uint32_t age_seconds)
  {
      ...
      fc::real128 apr_n( (asset( FBTC_BLOCKCHAIN_MAX_SHARES, apr.base_asset_id ) * apr).amount );
      fc::real128 apr_d( (asset( FBTC_BLOCKCHAIN_MAX_SHARES, apr.base_asset_id ) ).amount );
  ...
  asset market_engine_v7::get_interest_paid_v1(const asset& total_amount_paid, const price& apr, uint32_t age_seconds)
  {
      ...
      // INTEREST_PAID  = TOTAL_PAID - DELTA_PRINCIPLE
      fc::real128 total_paid( total_amount_paid.amount );
      fc::real128 apr_n( (asset( FBTC_BLOCKCHAIN_MAX_SHARES, apr.quote_asset_id ) * apr).amount );
      fc::real128 apr_d( (asset( FBTC_BLOCKCHAIN_MAX_SHARES, apr.quote_asset_id ) ).amount );
      fc::real128 iapr = apr_n / apr_d;
      fc::real128 age_sec(age_seconds);
      fc::real128 sec_per_year(365 * 24 * 60 * 60);
      fc::real128 percent_of_year = age_sec / sec_per_year;
