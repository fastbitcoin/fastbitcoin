...
asset balance_record::calculate_yield_v1( fc::time_point_sec now, share_type amount, share_type yield_pool, share_type share_supply )const
{
   ...
   if(  elapsed_time > fc::seconds( FBTC_BLOCKCHAIN_MIN_YIELD_PERIOD_SEC ) )
   {
      if( yield_pool > 0 && share_supply > 0 )
      {
          ...
          *  interest.
          */
         if( elapsed_time < fc::seconds( FBTC_BLOCKCHAIN_BLOCKS_PER_YEAR * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC ) )
         {
            ...
            yield /= (FBTC_BLOCKCHAIN_BLOCKS_PER_YEAR * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

            delta_yield *= elapsed_time.to_seconds();
            delta_yield /= (FBTC_BLOCKCHAIN_BLOCKS_PER_YEAR * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
...
{ try {
   ...
   if( eval_state.pending_state()->get_head_block_num() >= FBTC_V0_4_10_FORK_BLOCK_NUM )
   {
...
{ try {
   ...
   {
       FC_ASSERT( amount.amount >= FBTC_BLOCKCHAIN_MIN_BURN_FEE );
   }
