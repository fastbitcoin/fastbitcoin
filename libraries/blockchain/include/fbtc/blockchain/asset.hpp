...
namespace fbtc { namespace blockchain {
  ...
  struct price
  {
      ...
      fc::uint128_t ratio; // This is a DECIMAL FRACTION with denominator equal to FBTC_PRICE_PRECISION
   ...
   *  denominator.  Therefore  fbtc/usd and  usd/fbtc will
   *  always result in a price measured in usd/fbtc because
   ...
   *
   *  ie:  p = 3 usd/fbtc & a = 4 fbtc then result = 12 usd
   *  ie:  p = 3 usd/fbtc & a = 4 usd then result = 1.333 fbtc
   */
...
namespace fc
{
 //  void to_variant( const fbtc::blockchain::asset& var,  variant& vo );
 //  void from_variant( const variant& var,  fbtc::blockchain::asset& vo );
   void to_variant( const fbtc::blockchain::price& var,  variant& vo );
   void from_variant( const variant& var,  fbtc::blockchain::price& vo );
}

#include <fc/reflect/reflect.hpp>
FC_REFLECT( fbtc::blockchain::asset, (amount)(asset_id) );
FC_REFLECT( fbtc::blockchain::price, (ratio)(quote_asset_id)(base_asset_id) );
