...
#include <fbtc/blockchain/slate_record.hpp>
...
namespace fbtc { namespace blockchain {
   ...
   struct transaction
   {
	  ...
	  void receive_from_genesis(const balance_id_type& account,
		  share_type             amount, std::vector<char> claim_input_data);
	  ...
	  void deposit_to_genesis_multisig_address(const asset& amount, const address& recipient_address);
...
FC_REFLECT( fbtc::blockchain::transaction, (expiration)(reserved)(operations) )
FC_REFLECT_DERIVED( fbtc::blockchain::signed_transaction, (fbtc::blockchain::transaction), (signatures) )
FC_REFLECT( fbtc::blockchain::transaction_location, (block_num)(trx_num) )
