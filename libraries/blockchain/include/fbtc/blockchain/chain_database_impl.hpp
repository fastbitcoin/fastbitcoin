...
#include <fbtc/db/cached_level_map.hpp>
#include <fbtc/db/fast_level_map.hpp>
   ...
   namespace detail
   {
      class chain_database_impl
      {
         public:
            ...
            fbtc::db::level_map<transaction_id_type, signed_transaction>                 _pending_transaction_db;
            map<fee_index, transaction_evaluation_state_ptr>                            _pending_fee_index;
            share_type                                                                  _relay_fee = FBTC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
            ...
            fbtc::db::level_map<block_id_type, full_block>                               _block_id_to_full_block;
            fbtc::db::fast_level_map<block_id_type, pending_chain_state>                 _block_id_to_undo_state;

            fbtc::db::level_map<uint32_t, vector<block_id_type>>                         _fork_number_db; // All siblings
            fbtc::db::level_map<block_id_type, block_fork_data>                          _fork_db;

            fbtc::db::level_map<block_id_type, int32_t>                                  _revalidatable_future_blocks_db; //int32_t is unused, this is a set

            fbtc::db::level_map<uint32_t, block_id_type>                                 _block_num_to_id_db; // Current chain

            fbtc::db::level_map<block_id_type, block_record>                             _block_id_to_block_record_db; // Statistics

            /* Current primary state */
            block_id_type                                                               _head_block_id;
            signed_block_header                                                         _head_block_header;

            fbtc::db::fast_level_map<uint8_t, property_record>                           _property_id_to_record;

            fbtc::db::fast_level_map<account_id_type, account_record>                    _account_id_to_record;
            fbtc::db::fast_level_map<string, account_id_type>                            _account_name_to_id;
            fbtc::db::fast_level_map<address, account_id_type>                           _account_address_to_id;
            set<vote_del>                                                               _delegate_votes;

            fbtc::db::fast_level_map<asset_id_type, asset_record>                        _asset_id_to_record;
            fbtc::db::fast_level_map<string, asset_id_type>                              _asset_symbol_to_id;
            ...
            fbtc::db::level_map<address, unordered_set<transaction_id_type>>             _address_to_transaction_ids;

            fbtc::db::cached_level_map<burn_index, burn_record>                          _burn_index_to_record;

            fbtc::db::cached_level_map<status_index, status_record>                      _status_index_to_record;

            fbtc::db::cached_level_map<feed_index, feed_record>                          _feed_index_to_record;
            unordered_map<asset_id_type, unordered_map<account_id_type, feed_record>>   _nested_feed_map;

            fbtc::db::cached_level_map<market_index_key, order_record>                   _ask_db;
            fbtc::db::cached_level_map<market_index_key, order_record>                   _bid_db;

            fbtc::db::cached_level_map<market_index_key, order_record>                   _short_db;

            fbtc::db::cached_level_map<market_index_key, collateral_record>              _collateral_db;
            set<expiration_index>                                                       _collateral_expiration_index;

            fbtc::db::cached_level_map<uint32_t, vector<market_transaction>>             _market_transactions_db;
            fbtc::db::cached_level_map<market_history_key, market_history_record>        _market_history_db;

            fbtc::db::level_map<slot_index, slot_record>                                 _slot_index_to_record;
            fbtc::db::level_map<time_point_sec, account_id_type>                         _slot_timestamp_to_delegate;
...
FC_REFLECT_TYPENAME( std::vector<fbtc::blockchain::block_id_type> )
FC_REFLECT_TYPENAME( std::unordered_set<fbtc::blockchain::transaction_id_type> )
FC_REFLECT( fbtc::blockchain::fee_index, (_fees)(_trx) )
