...
{
   template<>
   struct hash<fbtc::blockchain::pts_address> 
   {
       public:
         size_t operator()(const fbtc::blockchain::pts_address &a) const 
         {
            size_t s;
...
FC_REFLECT( fbtc::blockchain::pts_address, (addr) )

namespace fc 
{ 
   void to_variant( const fbtc::blockchain::pts_address& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  fbtc::blockchain::pts_address& vo );
}
