...
FC_REFLECT( fbtc::blockchain::extended_public_key, (pub_key)(chain_code) )
FC_REFLECT( fbtc::blockchain::extended_private_key, (priv_key)(chain_code) )
FC_REFLECT( fbtc::blockchain::extended_address, (addr) )

namespace fc
{
   void to_variant( const fbtc::blockchain::extended_address& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  fbtc::blockchain::extended_address& vo );
}

namespace std
{
   template<>
   struct hash<fbtc::blockchain::extended_address>
   {
       public:
         size_t operator()(const fbtc::blockchain::extended_address &a) const
         {
            size_t s;
            fc::sha1::encoder enc;
            fc::raw::pack( enc, a );
