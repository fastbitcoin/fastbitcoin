...
#include <fbtc/blockchain/market_engine_v2.hpp>
...
namespace fbtc { namespace blockchain { namespace detail {
      ...
      // reproduce some quirky behavior in pre-FBTC_V0_4_10_FORK_BLOCK_NUM blocks.
         ...
         try {
             ...
             if( base_id == 0 )
             {
                if( quote_asset->is_market_issued() )
                {
                   ...
                   if( pending_block_num >= FBTC_V0_4_10_FORK_BLOCK_NUM )
                   {
                      if( median_price )
                ...
                else // we only liquidate fees collected for user issued assets
                {
                  ...
                  while( get_next_ask() )
                  {
                     assert(_pending_state->get_head_block_num() < FBTC_V0_4_10_FORK_BLOCK_NUM || _current_ask);
                     if (_pending_state->get_head_block_num() >= FBTC_V0_4_10_FORK_BLOCK_NUM && !_current_ask)
                     ...
                     if( (asset(quote_asset->collected_fees,quote_id) * _current_ask->get_price()).amount < (10000 * FBTC_BLOCKCHAIN_PRECISION) )
                        break;
             ...
             while( get_next_bid() && get_next_ask() )
             {
                ...
                if( _current_ask->type == cover_order && _current_bid->type == short_order )
                {
                   ...
                   if( pending_block_num >= FBTC_V0_4_12_FORK_BLOCK_NUM )
                   {
                       /**
                   ...
                   if( bid_quantity_xts.amount - xts_paid_by_short.amount < FBTC_BLOCKCHAIN_PRECISION )
                ...
                else if( _current_ask->type == cover_order && _current_bid->type == bid_order )
                {
                   ...
                   if( pending_block_num >= FBTC_V0_4_9_FORK_2_BLOCK_NUM )
                   {
                ...
                else if( _current_ask->type == ask_order && _current_bid->type == short_order )
                {
                   ...
                   // rounding errors go into collateral, round to the nearest 1 XTS
                   if( bid_quantity_xts.amount - quantity_xts.amount < FBTC_BLOCKCHAIN_PRECISION )
                      xts_paid_by_short = bid_quantity_xts;
             ...
             {
                if( median_price && pending_block_num >= FBTC_V0_4_12_FORK_BLOCK_NUM )
                {
                   market_stat->center_price = *median_price;
                }
                else
                {
                   ...
                   market_stat->center_price.ratio *= (FBTC_BLOCKCHAIN_BLOCKS_PER_HOUR-1);
      ...
      { try {
          ...
          FC_ASSERT( mtrx.ask_paid.amount >= (xts_paid_by_short.amount - FBTC_BLOCKCHAIN_PRECISION), "", ("mtrx",mtrx)("xts_paid_by_short",xts_paid_by_short) );
      ...
      { try {
         ...
         if( _ask_itr.valid() )
         {
            ...
            if( _pending_state->get_head_block_num() >= FBTC_V0_4_10_FORK_BLOCK_NUM )
               return _current_ask.valid();
