...
{ try {
    ...
    if( this->maximum_share_supply <= 0 || this->maximum_share_supply > FBTC_BLOCKCHAIN_MAX_SHARES )
...
{ try {
    ...
    if( this->max_supply.valid() )
    {
        ...
        if( *this->max_supply < current_asset_record->current_supply || *this->max_supply > FBTC_BLOCKCHAIN_MAX_SHARES )
    ...
    if( this->market_fee_rate.valid() )
    {
        ...
        if( *this->market_fee_rate > FBTC_BLOCKCHAIN_MAX_UIA_MARKET_FEE_RATE )
