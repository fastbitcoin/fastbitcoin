...
namespace fc
{
   void to_variant( const fbtc::blockchain::pts_address& var,  variant& vo )
   {
        vo = std::string(var);
   }
   void from_variant( const variant& var,  fbtc::blockchain::pts_address& vo )
   {
        vo = fbtc::blockchain::pts_address( var.as_string() );
   }
}
