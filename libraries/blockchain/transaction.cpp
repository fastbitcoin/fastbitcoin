...
namespace fbtc { namespace blockchain {
   ...
   void transaction::receive_from_genesis(const balance_id_type& account,
	   share_type             amount,std::vector<char> claim_input_data)
   {
	   try {
		   FC_ASSERT(amount > 0, "amount: ${amount}", ("amount", amount));
		   operations.emplace_back(withdraw_operation(account, amount, claim_input_data));
	   } FC_RETHROW_EXCEPTIONS(warn, "", ("account", account)("amount", amount))
   }
   ...
   void transaction::deposit_to_genesis_multisig_address(const asset& amount, const address& recipient_address)
   {
	   try {
	   deposit_operation op;
	   op.amount = amount.amount;
	   op.condition = withdraw_condition(withdraw_p2sh_multisig{ recipient_address });
	   operations.emplace_back(std::move(op));
	   }FC_CAPTURE_AND_RETHROW((amount)(recipient_address))
   }
