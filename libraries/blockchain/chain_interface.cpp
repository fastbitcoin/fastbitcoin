...
namespace fbtc { namespace blockchain {
   ...
   { try {
      if( name.size() < FBTC_BLOCKCHAIN_MIN_NAME_SIZE ) return false;
      if( name.size() > FBTC_BLOCKCHAIN_MAX_NAME_SIZE ) return false;
   ...
   { try {
       if( get_head_block_num() < FBTC_V0_5_0_FORK_BLOCK_NUM )
           return is_valid_symbol_name_v1( symbol );

       FC_ASSERT( symbol != "FBTCX" );

       if( symbol.size() < FBTC_BLOCKCHAIN_MIN_SYMBOL_SIZE)
       ...
       if (dots == 0)
       {
         if (position > FBTC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
           FC_ASSERT(false, "Symbol name too large");
       }
       else //dots == 1 means hierarchial asset name
       {
         if (position - dot_position - 1> FBTC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE)
           FC_ASSERT(false, "Sub-symbol name too large");
         if( symbol.size() > FBTC_BLOCKCHAIN_MAX_SYMBOL_SIZE)
           FC_ASSERT(false, "Symbol name too large");
       }
   ...
   {
	   return share_type(0);
       share_type pay_per_block = FBTC_MAX_DELEGATE_PAY_PER_BLOCK;
   ...
   share_type chain_interface::get_delegate_registration_fee( uint8_t pay_rate )const
   {
	   ...
	   return share_type(100 * FBTC_BLOCKCHAIN_PRECISION);
       ...
       const share_type max_pay_per_two_weeks = max_total_pay_per_two_weeks / FBTC_BLOCKCHAIN_NUM_DELEGATES;
   ...
   {
       if( get_head_block_num() < FBTC_V0_9_0_FORK_BLOCK_NUM )
           return get_asset_registration_fee_v2( symbol_length );
	   return share_type(10 * FBTC_BLOCKCHAIN_PRECISION);
       // TODO: Add #define's for these fixed prices
       static const share_type long_symbol_price = 5000 * FBTC_BLOCKCHAIN_PRECISION; // $100 at $0.02/share
