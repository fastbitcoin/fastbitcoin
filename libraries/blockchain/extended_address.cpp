...
namespace fbtc { namespace blockchain {
  ...
  fc::ecc::public_key extended_private_key::get_public_key()const
  {
   ...
   { try {
      ...
      uint32_t checksum = 0;
      std::string prefix( FBTC_ADDRESS_PREFIX );
   ...
   extended_address::operator std::string()const
   {
      ...
      return FBTC_ADDRESS_PREFIX + fc::to_base58( data.data(), data.size() );
   }
...
namespace fc
{
   void to_variant( const fbtc::blockchain::extended_address& ext_addr,  fc::variant& vo )
   {
   ...
   {
      vo = fbtc::blockchain::extended_address( var.as_string() );
   }
}
