...
namespace fbtc { namespace blockchain {
   ...
   { try {
       ...
       if( pending_state()->get_head_block_num() >= FBTC_V0_9_2_FORK_BLOCK_NUM )
       {
           for( const auto& op : trx.operations )
           {
               if( operation_type_enum( op.type ) == cover_op_type )
               {
                   FC_ASSERT( fees_paid[ 0 ] <= FBTC_BLOCKCHAIN_PRECISION );
               }
           }
       }
   } FC_CAPTURE_AND_RETHROW() }
   ...
   { try {
      ...
      try {
        ...
        {
           if(pending_state()->get_head_block_num()>1&&( pending_state()->now() > trx_arg.expiration || pending_state()->get_head_block_num() >= FBTC_V0_4_21_FORK_BLOCK_NUM ))
           {
        ...
        if( (pending_state()->now() + FBTC_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration )
           FC_CAPTURE_AND_THROW( invalid_transaction_expiration, (trx_arg)(pending_state()->now()) );

        if( pending_state()->get_head_block_num() >= FBTC_V0_4_26_FORK_BLOCK_NUM )
        {
   ...
   } FC_CAPTURE_AND_RETHROW( (trx_arg.id()) ) }
