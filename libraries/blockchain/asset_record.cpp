...
namespace fbtc { namespace blockchain {
    ...
    { try {
        ...
        if( satoshis > FBTC_BLOCKCHAIN_MAX_SHARES )
            FC_CAPTURE_AND_THROW( amount_too_large, (satoshis)(FBTC_BLOCKCHAIN_MAX_SHARES) );
    ...
    { try {
        ...
        FC_ASSERT( max_supply >= 0 && max_supply <= FBTC_BLOCKCHAIN_MAX_SHARES );
        FC_ASSERT( withdrawal_fee >= 0 && withdrawal_fee <= max_supply );
        FC_ASSERT( market_fee_rate <= FBTC_BLOCKCHAIN_MAX_UIA_MARKET_FEE_RATE );
