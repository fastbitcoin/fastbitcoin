...
{ try {
    ...
    if( delta_amount.amount > 0 ) // Withdraw from transaction and deposit to order
    {
        ...
        if( eval_state.pending_state()->get_head_block_num() >= FBTC_V0_9_1_FORK_BLOCK_NUM )
...
{ try {
    ...
    const time_point_sec start_time = current_position->expiration - fc::seconds( FBTC_BLOCKCHAIN_MAX_SHORT_PERIOD_SEC );
    ...
    else
    {
        ...
        const price new_call_price = asset( current_position->payoff_balance, this->cover_index.order_price.quote_asset_id )
                                     / asset( (current_position->collateral_balance * FBTC_BLOCKCHAIN_MCALL_D2C_NUMERATOR)
                                              / FBTC_BLOCKCHAIN_MCALL_D2C_DENOMINATOR, this->cover_index.order_price.base_asset_id );
