...
#include <fbtc/blockchain/pending_chain_state.hpp>
...
namespace fbtc { namespace blockchain {
   ...
   asset balance_record::calculate_yield( fc::time_point_sec now, share_type amount, share_type yield_pool, share_type share_supply )const
   {
      ...
      if( elapsed_time <= fc::seconds( FBTC_BLOCKCHAIN_MIN_YIELD_PERIOD_SEC ) )
      ...
      // FBTC_BLOCKCHAIN_MAX_SHARES * 1000000 * FBTC_BLOCKCHAIN_MAX_SHARES
       ...
       *  interest.
       */
      if( elapsed_time < fc::seconds( FBTC_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC ) )
      {
         ...
         // FBTC_BLOCKCHAIN_MAX_SHARES * 1000000 * FBTC_BLOCKCHAIN_MAX_SHARES * 8
         // thus it cannot overflow
         //
         ...
         yield /= FBTC_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC;

         delta_yield *= elapsed_time.to_seconds();
         delta_yield /= FBTC_BLOCKCHAIN_MAX_YIELD_PERIOD_SEC;
   ...
   { try {
       if( eval_state.pending_state()->get_head_block_num() < FBTC_V0_9_0_FORK_BLOCK_NUM )
   ...
   { try {
      ...
      if( !authority_is_retracting )
      {
         ...
         switch( (withdraw_condition_types)current_balance_record->condition.type )
         {
			...
			case withdraw_p2sh_multisig_type:
			{
				auto p2sh_multisig = current_balance_record->condition.as<withdraw_p2sh_multisig>();
				...
				address redeem_address(scriptid, address_type::multisig_address);
				...
				bool ret = fbtc::utilities::redeem_decode(claim_input_data,required,total,all_public_key);
				if (!ret)
				{
					FC_CAPTURE_AND_THROW(invalid_redeem_script);
				}
   ...
   { try {
      ...
      if( amount.asset_id == 0 )
      {
          ...
          const share_type required_fee = message_kb * FBTC_BLOCKCHAIN_MIN_BURN_FEE;
   ...
   { try {
      ...
      FC_ASSERT( current_balance_record->condition.asset_id == 0, "Only FBTC balances can have restricted owners." );
      ...
      auto fee = FBTC_BLOCKCHAIN_PRECISION / 2;
      ...
      else // NOT this->new_restricted_owner.valid() || (this->new_restricted_owner == new_restricted_owner)
      {
          ...
          FC_ASSERT( eval_state.pending_state()->now().sec_since_epoch() - last_update_secs
                     >= FBTC_BLOCKCHAIN_VOTE_UPDATE_PERIOD_SEC,
