...
#include <fbtc/blockchain/market_engine_v1.hpp>
#include <fbtc/blockchain/market_engine_v2.hpp>
#include <fbtc/blockchain/market_engine_v3.hpp>
#include <fbtc/blockchain/market_engine_v4.hpp>
#include <fbtc/blockchain/market_engine_v5.hpp>
...
{ try {
      ...
      if( pending_state->get_head_block_num() >= FBTC_V0_4_16_FORK_BLOCK_NUM )
      {
...
{ try {
  ...
  if( pending_block_num == FBTC_V0_8_0_FORK_BLOCK_NUM )
  {
  ...
  else if( pending_block_num == FBTC_V0_4_21_FORK_BLOCK_NUM )
  {
     market_engine_v6 engine( pending_state, *this );
     engine.cancel_all_shorts();
     market_transactions.insert( market_transactions.end(), engine._market_transactions.begin(), engine._market_transactions.end() );
  }
  else if( pending_block_num == FBTC_V0_4_17_FORK_BLOCK_NUM )
  {
     market_engine_v4 engine( pending_state, *this );
     engine.cancel_all_shorts( self->get_block_header( FBTC_V0_4_16_FORK_BLOCK_NUM ).timestamp );
     market_transactions.insert( market_transactions.end(), engine._market_transactions.begin(), engine._market_transactions.end() );
  }
  else if( pending_block_num == FBTC_V0_4_16_FORK_BLOCK_NUM )
  {
  ...
  else
  {
      ...
      for( const auto& market_pair : dirty_markets )
      {
         ...
         if( pending_block_num >= FBTC_V0_4_21_FORK_BLOCK_NUM )
         {
            market_engine_v7 engine( pending_state, *this );
         ...
         else if( pending_block_num >= FBTC_V0_4_19_FORK_BLOCK_NUM )
         {
            market_engine_v6 engine( pending_state, *this );
            if( engine.execute( market_pair.first, market_pair.second, timestamp ) )
            {
         ...
         else if( pending_block_num >= FBTC_V0_4_13_FORK_BLOCK_NUM )
         {
         ...
         else if( pending_block_num >= FBTC_V0_4_0_FORK_BLOCK_NUM )
         {
  ...
  if( pending_block_num < FBTC_V0_4_9_FORK_BLOCK_NUM )
