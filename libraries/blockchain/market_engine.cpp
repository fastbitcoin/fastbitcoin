...
{ try {
    ...
    call_collateral.amount *= FBTC_BLOCKCHAIN_MCALL_D2C_NUMERATOR;
    call_collateral.amount /= FBTC_BLOCKCHAIN_MCALL_D2C_DENOMINATOR;
...
{ try {
    ...
    {
        FC_ASSERT( base_asset.market_fee_rate <= FBTC_BLOCKCHAIN_MAX_UIA_MARKET_FEE_RATE );
        issuer_fee = mtrx.bid_received.amount * base_asset.market_fee_rate;
        issuer_fee /= FBTC_BLOCKCHAIN_MAX_UIA_MARKET_FEE_RATE;
    }
...
{ try {
    ...
    {
        FC_ASSERT( quote_asset.market_fee_rate <= FBTC_BLOCKCHAIN_MAX_UIA_MARKET_FEE_RATE );
