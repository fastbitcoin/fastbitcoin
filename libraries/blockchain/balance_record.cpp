...
namespace fbtc { namespace blockchain {
   ...
   balance_record::balance_record( const address& owner, const asset& balance_arg, slate_id_type delegate_id, bool btc_p2sh_address/* = false*/)
   {
       balance = balance_arg.amount;
	   if (btc_p2sh_address) {
		   condition = withdraw_condition(withdraw_p2sh_multisig(owner), balance_arg.asset_id, delegate_id);
	   }
	   else {
