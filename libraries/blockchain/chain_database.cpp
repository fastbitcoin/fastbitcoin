...
#include <fbtc/blockchain/checkpoints.hpp>
...
#include <fbtc/blockchain/market_engine.hpp>
#include <fbtc/blockchain/snapshot_state.hpp>
#include <fbtc/blockchain/time.hpp>
...
namespace fbtc { namespace blockchain {
   ...
   namespace detail
   {
      ...
      { try {
          ...
          if( !version_record.valid() || version_record->value.as_uint64() != FBTC_BLOCKCHAIN_DATABASE_VERSION )
              return true;
      ...
      { try {
         ...
         if( chain_id == FBTC_EXPECTED_CHAIN_ID )
             chain_id = FBTC_DESIRED_CHAIN_ID;

         self->set_chain_id( chain_id );

         // Check genesis state
         FC_ASSERT( config.delegates.size() >= FBTC_BLOCKCHAIN_NUM_DELEGATES,
                    "genesis.json does not contain enough initial delegates!",
                    ("required",FBTC_BLOCKCHAIN_NUM_DELEGATES)("provided",config.delegates.size()) );
         ...
         {
             static const vector<string> fbtc_prefixes{ "FBTC", "KEY", "DVS", "XTS" };
             try
             {
             ...
             {
                 for( const string& prefix : fbtc_prefixes )
                 {
                     if( raw_address.find( prefix ) == 0 )
                     {
                         return address( FBTC_ADDRESS_PREFIX + raw_address.substr( prefix.size() ) );
                     }
                 }
             }
         ...
         for( const auto& genesis_balance : config.initial_balances )
         {
			...
			address temp_addr;
			const int AddrType = temp_addr.judge_addr_type(genesis_balance.raw_address);
			if (AddrType == fbtc::blockchain::address_type::multisig_address){
				balance_record initial_balance(addr, asset(genesis_balance.balance, 0), 0,true);
         ...
         base_asset.symbol = FBTC_BLOCKCHAIN_SYMBOL;
         base_asset.issuer_id = asset_record::god_issuer_id;
         base_asset.name = FBTC_BLOCKCHAIN_NAME;
         base_asset.description = FBTC_BLOCKCHAIN_DESCRIPTION;
         base_asset.precision = FBTC_BLOCKCHAIN_PRECISION;
         base_asset.max_supply = FBTC_BLOCKCHAIN_MAX_SHARES;
         ...
         for( const genesis_asset& asset : config.market_assets )
         {
             ...
             rec.max_supply = FBTC_BLOCKCHAIN_MAX_SHARES;
      ...
      { try {
          if( pending_state->get_head_block_num() < FBTC_V0_6_0_FORK_BLOCK_NUM )
          ...
          static const uint32_t blocks_per_two_weeks = 14 * FBTC_BLOCKCHAIN_BLOCKS_PER_DAY;
      ...
      { try {
          ...
          if( block_num > FBTC_BLOCKCHAIN_MAX_UNDO_HISTORY )
          {
              const uint32_t old_block_num = block_num - FBTC_BLOCKCHAIN_MAX_UNDO_HISTORY;
      ...
      { try {
          ...
          if( block_digest.timestamp.sec_since_epoch() % FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0 )
          ...
          fc::time_point_sec now = fbtc::blockchain::now();
          const uint32_t delta_seconds = (block_digest.timestamp - now).to_seconds();
          if( block_digest.timestamp >  (now + FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC*2) )
             FC_CAPTURE_AND_THROW( time_in_future, (block_digest.timestamp)(now)(delta_seconds) );

          if( block_digest.block_num >= FBTC_V0_9_0_FORK_BLOCK_NUM )
          {
      ...
      /**
       *  A block should be produced every FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
      ...
      { try {
          ...
          if( head_block.block_num > 0 ) block_timestamp = head_block.timestamp + FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
          else block_timestamp = block_header.timestamp;
          const auto& active_delegates = self->get_active_delegates();

          for( ; block_timestamp < block_header.timestamp; block_timestamp += FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
          {
      ...
      void chain_database_impl::update_active_delegate_list( const uint32_t block_num,
                                                             const pending_chain_state_ptr& pending_state )const
      { try {
          if( pending_state->get_head_block_num() < FBTC_V0_9_0_FORK_BLOCK_NUM )
              return update_active_delegate_list_v1( block_num, pending_state );

          if( block_num % FBTC_BLOCKCHAIN_NUM_DELEGATES != 0 )
              return;
          ...
          const pending_chain_state_ptr& pending_state ) const
      {
          ...
          for( fbtc::db::cached_level_map< market_index_key, order_record >::iterator
          ...
          for( fbtc::db::cached_level_map< market_index_key, collateral_record >::iterator
          ...
          for( auto it=mkt_to_best_bid.begin(); it != mkt_to_best_bid.end(); it++ )
          {
              ...
              auto get_order_type = [&]( const market_order& order ) -> std::string
              {
                  switch( order.type )
                  {
                      case fbtc::blockchain::null_order:
                          return "null_order";
                      case fbtc::blockchain::bid_order:
                          return "bid_order";
                      case fbtc::blockchain::ask_order:
                          return "ask_order";
                      case fbtc::blockchain::short_order:
                          return "short_order";
                      case fbtc::blockchain::cover_order:
                          return "cover_order";
                      default:
                          return "<unknown>";
                  }
      ...
      { try {
         ...
         try
         {
            ...
            if( block_data.block_num < FBTC_V0_4_9_FORK_BLOCK_NUM )
            ...
            if( block_data.block_num == FBTC_V0_4_16_FORK_BLOCK_NUM )
            {
            ...
            else if( block_data.block_num == FBTC_V0_4_17_FORK_BLOCK_NUM
                     || block_data.block_num == FBTC_V0_4_21_FORK_BLOCK_NUM
                     || block_data.block_num == FBTC_V0_4_24_FORK_BLOCK_NUM
                     || block_data.block_num == FBTC_V0_9_0_FORK_BLOCK_NUM )
            {
                ...
                auto defbtc = self->calculate_defbtc( false );
                ...
                for( auto& record : records )
                {
                    ...
                    {
                        share_type debt = defbtc[ record.id ];
                        if( supply != debt )
                        {
            ...
            if( block_data.block_num == FBTC_V0_4_24_FORK_BLOCK_NUM )
            {
            ...
            if( block_data.block_num == FBTC_V0_9_0_FORK_BLOCK_NUM )
            {
                static const fc::uint128 max_apr = fc::uint128( FBTC_BLOCKCHAIN_MAX_SHORT_APR_PCT ) * FC_REAL128_PRECISION / 100;
         ...
         if( (blockchain::now() - block_data.timestamp).to_seconds() < FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
         {
      ...
      { try {
         ...
         fbtc::blockchain::pending_chain_state_ptr undo_state_ptr = std::make_shared<fbtc::blockchain::pending_chain_state>( undo_state );
   ...
   {
       return get_delegates_by_vote( 0, FBTC_BLOCKCHAIN_NUM_DELEGATES );
   }
   ...
   { try {
      ...
      try
      {
          ...
          else
          {
              ...
              my->open_database( data_dir );
              store_property_record( property_id_type::database_version, variant( FBTC_BLOCKCHAIN_DATABASE_VERSION ) );
              ...
              const auto insert_block = [&]( const full_block& block )
              {
                  if( blocks_indexed % 200 == 0 )
                  {
                      ...
                      else
                      {
                          ...
                          progress = float( blocks_indexed * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC ) / seconds;
                      }
              ...
              else
              {
                  ...
                  if( last_known_block_num > FBTC_BLOCKCHAIN_MAX_UNDO_HISTORY )
                      my->_min_undo_block = last_known_block_num - FBTC_BLOCKCHAIN_MAX_UNDO_HISTORY;
   ...
   { try {
      ...
      auto delegate_pos = slot_number % FBTC_BLOCKCHAIN_NUM_DELEGATES;
   ...
   { try {
      ...
      if( next_block_time <= now() ) next_block_time += FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
      auto last_block_time = next_block_time + (FBTC_BLOCKCHAIN_NUM_DELEGATES * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

      auto active_delegates = get_active_delegates();
      for( ; next_block_time < last_block_time; next_block_time += FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
      {
   ...
   { try {
      ...
      if( head_block_num > FBTC_BLOCKCHAIN_MAX_UNDO_HISTORY &&
          block_data.block_num <= (head_block_num - FBTC_BLOCKCHAIN_MAX_UNDO_HISTORY) )
      {
        elog( "block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
               "our undo history would allow us to switch to (current head block is number ${head_block_num}, undo history is ${undo_history})",
                           ...
                           ("head_block_num", head_block_num)("undo_history", FBTC_BLOCKCHAIN_MAX_UNDO_HISTORY));
  ...
  std::vector<block_id_type> chain_database::get_fork_history( const block_id_type& id )
  {
   ...
   { try {
       for( auto iter = my->_balance_id_to_record.begin();
            iter.valid(); ++iter )
       {
           callback( iter.value());
       }
   ...
   { try {
      ...
      if( !override_limits )
      {
         if( my->_pending_fee_index.size() > FBTC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE )
         {
             auto overage = my->_pending_fee_index.size() - FBTC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
      ...
      return eval_state;
   } FC_CAPTURE_AND_RETHROW( (trx.id())(override_limits) ) }
   ...
   int32_t	chain_database::get_pending_transactions_size()const
   {
	   return  my->_pending_fee_index.size();
   }

   full_block chain_database::generate_block( const time_point_sec block_timestamp, const delegate_config& config )
   { try {
      const time_point start_time = time_point::now();
      //FC_ASSERT( block_timestamp < (fc::time_point_sec() + fc::seconds( 1444741200 )), "Upgrade to FBTC 2.0" );

      const pending_chain_state_ptr pending_state = std::make_shared<pending_chain_state>( shared_from_this() );
      if( pending_state->get_head_block_num() >= FBTC_V0_4_9_FORK_BLOCK_NUM )
          ...
          for( const transaction_evaluation_state_ptr& item : pending_trx )
          {
              ...
              try
              {
                  ...
                  auto pending_trx_state = std::make_shared<pending_chain_state>( pending_state );
                  {
					  ...
					  //trx_eval_state->_skip_signature_check = false;
   ...
   { try {
    ...
    string chain_database::export_fork_graph( uint32_t start_block, uint32_t end_block, const fc::path& filename )const
    {
      ...
      for( auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr )
      {
        ...
        {
          unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
   ...
   { try {
      ...
      const auto now = fbtc::blockchain::now();
      if( head_num < 1 )
      {
          return 0;
      }
      else if( head_num <= FBTC_BLOCKCHAIN_NUM_DELEGATES )
      {
         // what percent of the maximum total blocks that could have been produced
         // have been produced.
         const auto expected_blocks = (now - get_block_header( 1 ).timestamp).to_seconds() / FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
      ...
      else
      {
         ...
         const auto starting_time = get_block_header( head_num - FBTC_BLOCKCHAIN_NUM_DELEGATES ).timestamp;
         const auto expected_production = (now - starting_time).to_seconds() / FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
         return 100*double( FBTC_BLOCKCHAIN_NUM_DELEGATES ) / expected_production;
      }
   } FC_RETHROW_EXCEPTIONS( warn, "" ) }
   ...
   { try {
       ...
       for( auto iter = my->_balance_id_to_record.begin();
            iter.valid(); ++iter )
       {
           const balance_record& record = iter.value();
   ...
   { try {
       ...
       snapshot.max_core_supply = calculate_max_core_supply( FBTC_MAX_DELEGATE_PAY_PER_BLOCK );
       ...
       const auto scan_account = [ & ]( const account_record& record )
       {
           ...
           if( record.is_delegate() )
           {
               ...
               {
                   snapshot_account.daily_pay = FBTC_BLOCKCHAIN_BLOCKS_PER_DAY * FBTC_MAX_DELEGATE_PAY_PER_BLOCK * record.delegate_info->pay_rate
                                                / FBTC_BLOCKCHAIN_NUM_DELEGATES / 100;
               }
           }
       };
       ...
       for( auto iter = my->_collateral_db.begin(); iter.valid(); ++iter )
       {
           ...
           snapshot_asset.defbtc[ index.owner ].collateral += collateral.collateral_balance;
           snapshot_asset.defbtc[ index.owner ].debt += collateral.payoff_balance;
       }
       ...
       for( auto& item : snapshot.assets )
       {
           ...
           for( const auto& item : asset.defbtc )
           {
           ...
           if( total_supply != total_debt )
           {
               ...
               for( auto& item : debt_list )
                   asset.defbtc[ item.first ] = item.second;
       ...
       for( const auto& item : snapshot.assets )
       {
           ...
           snapshot.summary.num_collateral_positions += asset.defbtc.size();
       }
       ...
       for( const auto& item : snapshot.balances )
       {
           ...
           if( balances.vesting.valid() )
           {
               ...
               vesting_balance.asset_symbol = FBTC_BLOCKCHAIN_SYMBOL;
       ...
       for( const auto& item : snapshot.assets )
       {
           ...
           for( const auto& debt_item : snapshot_asset.defbtc )
           {
   ...
   { try {
       ...
       for( auto iter = my->_balance_id_to_record.begin(); iter.valid(); ++iter )
       {
           const balance_record& balance = iter.value();
   ...
   unordered_map<asset_id_type, share_type> chain_database::calculate_defbtc( bool include_interest )const
   { try {
       unordered_map<asset_id_type, share_type> totals;
       ...
       for( auto iter = my->_collateral_db.begin(); iter.valid(); ++iter )
       {
           ...
           if( include_interest )
           {
               const time_point_sec position_start_time = collateral.expiration - FBTC_BLOCKCHAIN_MAX_SHORT_PERIOD_SEC;
   ...
   { try {
       ...
       for( ; pay_per_block > 0; pay_per_block /= 2 )
       {
           ...
           max_supply += interval_duration * pay_per_block / FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
   ...
   { try {
       ...
       if( prices.size() >= FBTC_BLOCKCHAIN_MIN_FEEDS )
       {
