...
#include <fbtc/utilities/string_escape.hpp>
...
std::string type_mapping::create_value_of_type_from_variant(const fc::variant& value)
{
  ...
  default_value_as_variant << "fc::json::from_string(" << fbtc::utilities::escape_string_for_c_source_code(fc::json::to_string(value)) << ")";
...
struct method_description
{
  ...
  bool is_const;
  fbtc::api::method_prerequisites prerequisites; // actually, a bitmask of method_prerequisites
...
private:
  ...
  fbtc::api::method_prerequisites load_prerequisites(const fc::variant& json_prerequisites);
...
fbtc::api::method_prerequisites api_generator::load_prerequisites(const fc::variant& json_prerequisites)
{
  int result = 0;
  std::vector<fbtc::api::method_prerequisites> prereqs = json_prerequisites.as<std::vector<fbtc::api::method_prerequisites> >();
  for (fbtc::api::method_prerequisites prereq : prereqs)
    result |= prereq;
  return (fbtc::api::method_prerequisites)result;
}
...
void api_generator::write_generated_file_header(std::ostream& stream)
{
  ...
  stream << "//          generator (fbtc_api_generator.cpp) itself\n";
  stream << "//\n";
}
...
{
  fc::path api_description_header_path = api_description_output_dir / "include" / "fbtc" / "api";
  ...
  interface_file << "namespace fbtc { namespace api {\n\n";
  ...
  interface_file << "  };\n\n";
  interface_file << "} } // end namespace fbtc::api\n";
}
...
void api_generator::generate_rpc_client_files(const fc::path& rpc_client_output_dir, const std::string& generated_filename_suffix)
{
  ...
  fc::path client_header_path = rpc_client_output_dir / "include" / "fbtc" / "rpc_stubs";
  ...
  header_file << "#include <fbtc/api/" << _api_classname << ".hpp>\n\n";
  header_file << "namespace fbtc { namespace rpc_stubs {\n\n";
  header_file << "  class " << client_classname << " : public fbtc::api::" << _api_classname << "\n";
  header_file << "  {\n";
  header_file << "  public:\n";
  ...
  header_file << "  };\n\n";
  header_file << "} } // end namespace fbtc::rpc_stubs\n";

  write_generated_file_header(cpp_file);
  cpp_file << "#define DEFAULT_LOGGER \"rpc\"\n";
  cpp_file << "#include <fbtc/rpc_stubs/" << client_classname << ".hpp>\n";
  cpp_file << "#include <fbtc/api/conversion_functions.hpp>\n\n";
  cpp_file << "namespace fbtc { namespace rpc_stubs {\n\n";
...
void api_generator::generate_prerequisite_checks_to_stream(const method_description& method, std::ostream& stream)
{
  ...
  else if (method.prerequisites & fbtc::api::wallet_open)
  {
    stream << "  verify_json_connection_is_authenticated(json_connection);\n";
    stream << "  verify_wallet_is_open();\n";
  }
  else if (method.prerequisites & fbtc::api::json_authenticated)
  {
    stream << "  verify_json_connection_is_authenticated(json_connection);\n";
  }

  if (method.prerequisites & fbtc::api::connected_to_network)
...
void api_generator::generate_rpc_server_files(const fc::path& rpc_server_output_dir, const std::string& generated_filename_suffix)
{
  ...
  fc::path server_header_path = rpc_server_output_dir / "include" / "fbtc" / "rpc_stubs";
  ...
  write_generated_file_header(header_file);
  header_file << "#pragma once\n";
  header_file << "#include <fbtc/api/api_metadata.hpp>\n";
  header_file << "#include <fbtc/api/common_api.hpp>\n";
  header_file << "#include <fc/rpc/json_connection.hpp>\n\n";
  header_file << "namespace fbtc { namespace rpc_stubs {\n";
  header_file << "  class " << server_classname << "\n";
  header_file << "  {\n";
  header_file << "  public:\n";
  header_file << "    virtual fbtc::api::common_api* get_client() const = 0;\n";
  ...
  header_file << "    virtual void store_method_metadata(const fbtc::api::method_data& method_metadata) = 0;\n";
  ...
  server_cpp_file << "#include <fbtc/rpc_stubs/" << server_classname << ".hpp>\n";
  server_cpp_file << "#include <fbtc/api/api_metadata.hpp>\n";
  server_cpp_file << "#include <fbtc/api/conversion_functions.hpp>\n";
  server_cpp_file << "#include <boost/bind.hpp>\n";
  write_includes_to_stream(server_cpp_file);
  server_cpp_file << "\n";
  server_cpp_file << "namespace fbtc { namespace rpc_stubs {\n\n";
  ...
  for (const method_description& method : _methods)
  {
    server_cpp_file << "  {\n";
    server_cpp_file << "    // register method " << method.name << "\n";
    server_cpp_file << "    fbtc::api::method_data " << method.name << "_method_metadata{\"" << method.name << "\", nullptr,\n";
    server_cpp_file << "      /* description */ " << fbtc::utilities::escape_string_for_c_source_code(method.brief_description) << ",\n";
    ...
    for (const parameter_description& parameter : method.parameters)
    {
      if (first_parameter)
        ...
        server_cpp_file << ",";
      server_cpp_file << "\n        {\"" << parameter.name << "\", \"" << parameter.type->get_type_name() <<  "\", fbtc::api::";
      if (parameter.default_value)
        server_cpp_file << "optional_positional, fc::variant(fc::json::from_string(" << fbtc::utilities::escape_string_for_c_source_code(fc::json::to_string(parameter.default_value)) << "))}";
      else
    ...
    if( !first_parameter )
      server_cpp_file << "\n      ";
    server_cpp_file << "},\n";
    server_cpp_file << "      /* prerequisites */ (fbtc::api::method_prerequisites) " << (int)method.prerequisites << ",\n";
    server_cpp_file << "      /* detailed description */ " << fbtc::utilities::escape_string_for_c_source_code(generate_detailed_description_for_method(method)) << ",\n";
  ...
  server_cpp_file << "\n";
  server_cpp_file << "} } // end namespace fbtc::rpc_stubs\n";
}
...
{
  cpp_file << "  fbtc::api::global_api_logger* glog = fbtc::api::global_api_logger::get_instance();\n"
              "  uint64_t call_id = 0;\n"
              "  fc::variants args;\n"
              "  if( glog != NULL )\n"
              "  {\n";
...
{
  fc::path client_header_path = client_output_dir / "include" / "fbtc" / "rpc_stubs";
  fc::create_directories(client_header_path); // creates dirs for both header and cpp
  ...
  interceptor_header_file << "#include <fbtc/api/api_metadata.hpp>\n";
  interceptor_header_file << "#include <fbtc/api/common_api.hpp>\n\n";

  interceptor_header_file << "namespace fbtc { namespace rpc_stubs {\n";
  interceptor_header_file << "  class " << interceptor_classname << " : public fbtc::api::common_api\n";
  interceptor_header_file << "  {\n";
  interceptor_header_file << "  protected:\n";
  interceptor_header_file << "    virtual fbtc::api::common_api* get_impl() const = 0;\n\n";
  ...
  interceptor_header_file << "  };\n\n";
  interceptor_header_file << "} } // end namespace fbtc::rpc_stubs\n";

  interceptor_cpp_file << "#define DEFAULT_LOGGER \"rpc\"\n";
#if FBTC_GLOBAL_API_LOG
  interceptor_cpp_file << "#include <fbtc/api/global_api_logger.hpp>\n";
  interceptor_cpp_file << "#include <fbtc/api/conversion_functions.hpp>\n";
#endif
  interceptor_cpp_file << "#include <fbtc/rpc_stubs/" << interceptor_classname << ".hpp>\n\n";
  interceptor_cpp_file << "namespace fbtc { namespace rpc_stubs {\n\n";
  ...
  interceptor_cpp_file << "} } // end namespace fbtc::rpc_stubs\n";
}
