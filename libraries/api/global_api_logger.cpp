...
namespace detail
{
    ...
    public:
        ...
        virtual uint64_t log_call_started( const fbtc::api::common_api* target, const fc::string& name, const fc::variants& args ) override;
    ...
    uint64_t global_api_logger_impl::log_call_started( const fbtc::api::common_api* target, const fc::string& name, const fc::variants& args )
    {
    ...
    void global_api_logger_impl::log_call_finished( uint64_t call_id, const fbtc::api::common_api* target, const fc::string& name, const fc::variants& args, const fc::variant& result )
    {
        if( name == "debug_get_client_name" )
            return;
        for( api_logger* logger : this->active_loggers )
