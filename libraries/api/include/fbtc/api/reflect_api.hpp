...
FC_API( fbtc::api::blockchain_api, 
        (about)
        (get_info)
        (validate_address)
        (blockchain_get_info)
...
FC_API( fbtc::api::wallet_api, (wallet_unlock) )
...
namespace fbtc { namespace api {
   ...
   struct login_api 
   {
      public:
         ...
         fc::api<fbtc::api::blockchain_api> get_blockchain()const
         {
            FC_ASSERT( _blockchain, "no permission to access blockchain" );
            return *_blockchain;
         }
         fc::api<fbtc::api::wallet_api>     get_wallet()const
         {
            FC_ASSERT( _wallet, "no permission to access wallet" );
            return *_wallet;
         }

         optional<fc::api<fbtc::api::blockchain_api>> _blockchain;
         optional<fc::api<fbtc::api::wallet_api>>     _wallet;
         map<string,permissions>                     _permissions;
         common_api* _capi = nullptr;
   };
} } // fbtc::api

FC_REFLECT( fbtc::api::permissions, (password)(access) );
FC_API( fbtc::api::login_api, (login)(get_blockchain)(get_wallet) )
