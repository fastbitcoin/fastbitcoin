...
#include <fbtc/wallet/wallet_db.hpp>
...
namespace fbtc { namespace wallet {
   ...
   namespace detail {
     class wallet_db_impl
     {
        public:
           ...
           fbtc::db::cached_level_map<int32_t,generic_wallet_record> _records;
           ...
           { try {
               ...
               // Cache address map
               self->btc_to_fbtc_address[ key_address ] = key_address;
               self->btc_to_fbtc_address[ address( pts_address( key_record.public_key, false, 0  ) ) ] = key_address; // Uncompressed BTC
               self->btc_to_fbtc_address[ address( pts_address( key_record.public_key, true,  0  ) ) ] = key_address; // Compressed BTC
               self->btc_to_fbtc_address[ address( pts_address( key_record.public_key, false, 56 ) ) ] = key_address; // Uncompressed PTS
               self->btc_to_fbtc_address[ address( pts_address( key_record.public_key, true,  56 ) ) ] = key_address; // Compressed PTS
   ...
   { try {
       FC_ASSERT( is_open() );
       const auto address_map_iter = btc_to_fbtc_address.find( derived_address );
       if( address_map_iter != btc_to_fbtc_address.end() )
       {
               ...
               if( wallet_record_type_enum( record.type ) == key_record_type )
               {
                   ...
                   if( key_record.has_private_key() )
                   {
                       ...
                       if( key_record.public_key != public_key )
                       {
                           ...
                           btc_to_fbtc_address.erase( key_address );
                           btc_to_fbtc_address.erase( address( pts_address( key_record.public_key, false, 0  ) ) );
                           btc_to_fbtc_address.erase( address( pts_address( key_record.public_key, true,  0  ) ) );
                           btc_to_fbtc_address.erase( address( pts_address( key_record.public_key, false, 56 ) ) );
                           btc_to_fbtc_address.erase( address( pts_address( key_record.public_key, true,  56 ) ) );
