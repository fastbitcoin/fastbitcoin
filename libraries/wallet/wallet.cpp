#include <fbtc/wallet/config.hpp>
...
namespace detail {
   ...
   public_key_type wallet_impl::deposit_from_transaction( signed_transaction& transaction, const asset& amount,
                                                          ...
                                                          const string& memo )
   { try {
       if( memo.size() > FBTC_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE )
   ...
   void wallet_impl::upgrade_version()
   {
       ...
       if( current_version > FBTC_WALLET_VERSION )
       {
           FC_THROW_EXCEPTION( unsupported_version, "Wallet version newer than client supports!",
                               ("wallet_version",current_version)("supported_version",FBTC_WALLET_VERSION) );
       }
       else if( current_version == FBTC_WALLET_VERSION )
       {
           return;
       }
       ...
       try
       {
           ...
           {
               self->set_transaction_fee( asset( FBTC_WALLET_DEFAULT_TRANSACTION_FEE ) );
           }

           if( current_version < 106 )
           {
               self->set_transaction_expiration( FBTC_WALLET_DEFAULT_TRANSACTION_EXPIRATION_SEC );
           }
           ...
           {
               self->set_version( FBTC_WALLET_VERSION );
                                            ...
                                            )
   {
      ...
      //
      // For example, 1 USD = 10000 USD-satoshi, 1 FBTC = 100000 FBTC-satoshi
      //
      // Thus, 0.02 USD / FBTC expressed as a ratio of satoshis would be
      //       0.02 USD / FBTC * (1 FBTC / 100000 FBTC-satoshi) * (10000 USD-satoshi / 1 USD) = 0.002 USD-satoshi per FBTC-satoshi.
      //
   ...
   { try {
      ...
      if( password.size() < FBTC_WALLET_MIN_PASSWORD_LENGTH )
   ...
   slate_record wallet_impl::get_delegate_slate( const vote_strategy strategy )const
   {
       ...
       {
           if( record.slate.size() >= FBTC_BLOCKCHAIN_MAX_SLATE_SIZE )
               break;
   ...
   { try {
      ...
      try
      {
          ...
          open( wallet_name );
          unlock( password, FBTC_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
      ...
      try
      {
          ...
          close();
          open( wallet_name );
          unlock( passphrase, FBTC_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
      ...
      try
      {
          ...
          const uint32_t max_timeout_seconds = std::numeric_limits<uint32_t>::max() - fbtc::blockchain::now().sec_since_epoch();
   ...
   bool	wallet::check_passphrase(const string& passphrase)
   {
	   try {
		   ...
		   if (passphrase.size() < FBTC_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
   ...
   { try {
      if( NOT is_open() ) FC_CAPTURE_AND_THROW( wallet_closed );
      if( NOT is_unlocked() ) FC_CAPTURE_AND_THROW( wallet_locked );
      if( new_passphrase.size() < FBTC_WALLET_MIN_PASSWORD_LENGTH ) FC_CAPTURE_AND_THROW( password_too_short );
   ...
   { try {
      ...
      auto key = fbtc::utilities::wif_to_key( wif_key );
      if( key.valid() )
      {
   ...
   { try {
      ...
      const auto version = fbtc::client::version_info()["client_version"].as_string();
   ...
   { try {
       ...
       for( const auto& item : items )
       {
           ...
           if( name.find( FBTC_ADDRESS_PREFIX ) != 0 )
               continue;
   ...
   wallet_transaction_record wallet::transfer_asset_to_address(
	   const asset& amount,
	   const string& from_account_name,
	   const std::map<std::string, asset>& all_receive_detail,
	   const string& memo,
	   const vote_strategy strategy,
	   bool sign,
	   int64_t reserve
   )
   {
	   try {
		   ...
		   for (auto& iter : all_receive_detail)
		   {
			   trx.deposit_to_address(iter.second, address(iter.first));
			   auto entry = ledger_entry();
			   entry.from_account = sender_public_key;
			   entry.amount = iter.second;
			   if (memo != "")
			   {
				   entry.memo = memo;
			   }
			   else
				   entry.memo = "To: " + string(iter.first).substr(0, 8) + "...";
			   try
			   {
				   auto acc_rec = get_account_for_address(address(iter.first));
				   if (acc_rec.valid())
				   {
					   entry.to_account = acc_rec->owner_key;
				   }
		   ...
		   return trans_entry;
	   } FC_CAPTURE_AND_RETHROW((amount)(from_account_name)(memo))
   }



   wallet_transaction_record wallet::transfer_asset_to_genesis_multisig_address(
	   const asset& amount,
	   const string& from_account_name,
	   const string& generic_recipient,
	   const string& memo,
   ...
   )
   {
	   try {
		   ...
		   trx.deposit_to_genesis_multisig_address(amount, address(generic_recipient));
		   //trx.deposit_to_address(asset(9223272036854775807, 0), address(generic_recipient));
		   //trx.deposit_to_address(asset(9223272036854775807), address(generic_recipient));
		   //trx.deposit_to_address(asset(200000000000002, 0), address(generic_recipient));
		   if (reserve != 0)
		   {
			   trx.expiration = blockchain::now() + get_transaction_expiration() + int64_t(reserve)%30;
		   }
		   else {
			   trx.expiration = blockchain::now() + get_transaction_expiration();
		   }
   ...
   )
   {
	   try {
		   ...
		   for (auto& iter : all_receive_detail)
		   {
			   trx.deposit_to_genesis_multisig_address(iter.second, address(iter.first));
			   auto entry = ledger_entry();
			   entry.from_account = sender_public_key;
			   entry.amount = iter.second;
			   if (memo != "")
		   ...
		   return trans_entry;
	   } FC_CAPTURE_AND_RETHROW((amount)(from_account_name)(all_receive_detail)(memo))
   }
   ...
   { try {
      ...
      if( amount.asset_id != asset_id_type( 0 ) )
      {
          ...
          catch( const fbtc::wallet::insufficient_funds& )
          {
...
//      required_signatures.insert( issuer_account->active_key() );
  ...
  //    if( sign )
   ...
   { try {
      ...
      asset fee( FBTC_WALLET_DEFAULT_TRANSACTION_FEE, 0 );
      try
      {
   ...
   { try {
       ...
       if( secs > FBTC_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC )
