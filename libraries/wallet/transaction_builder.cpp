...
void transaction_builder::set_wallet_implementation(std::unique_ptr<fbtc::wallet::detail::wallet_impl>& wimpl)
{
    _wimpl = wimpl.get();
}
...
transaction_builder& transaction_builder::deposit_asset_with_escrow(const fbtc::wallet::wallet_account_record& payer,
                                                        const fbtc::blockchain::account_record& recipient,
                                                        const fbtc::blockchain::account_record& escrow_agent,
...
{ try {
   ...
   if( memo.size() > FBTC_BLOCKCHAIN_MAX_MEMO_SIZE )
...
{ try {
   ...
   auto age_at_earliest_confirmation = _wimpl->_blockchain->now()
                                       + FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC
                                       - (*order->expiration - FBTC_BLOCKCHAIN_MAX_SHORT_PERIOD_SEC);
...
transaction_builder& transaction_builder::receive_from_genesis_balance(const address& from_address,const std::string from_redeem_script, asset& amount)
{
	try {
		// TODO ledger entries

		auto condition = withdraw_condition(withdraw_p2sh_multisig(from_address));
		auto temp_balance = balance_record(condition);
		auto obalance = _wimpl->_blockchain->get_balance_record(temp_balance.id());
		...
		if (obalance.valid())
		{
			...
			fbtc::utilities::hex2bin(from_redeem_script.data(), redeem_data, from_redeem_script.size(),data_len);
			int required, total;
			std::vector<fc::ecc::public_key> all_public_key;

			bool ret = fbtc::utilities::redeem_decode(redeem_data, required, total, all_public_key);
		...
		{
			auto balances = _wimpl->_blockchain->get_balances_for_address(address(from_address));
			FC_ASSERT(balances.size() > 0, "No balance with that ID or owner address!");
			auto balance = balances.begin()->second;
