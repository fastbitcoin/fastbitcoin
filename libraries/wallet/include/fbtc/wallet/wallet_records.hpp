...
#include <fbtc/blockchain/account_record.hpp>
#include <fbtc/blockchain/balance_record.hpp>
#include <fbtc/blockchain/extended_address.hpp>
...
namespace fbtc { namespace wallet {
   ...
   struct account_data : public fbtc::blockchain::account_record
   {
       ...
       // Kept in FBTC for upgrading old wallet versions; remove after 1.0
       int8_t           approved = 0;
   };
...
FC_REFLECT_ENUM( fbtc::wallet::wallet_record_type_enum,
...
FC_REFLECT_ENUM( fbtc::wallet::property_enum,
        (version)
        (next_record_number)
        (next_child_key_index)
        (automatic_backups)
...
FC_REFLECT( fbtc::wallet::wallet_property,
        (key)
        (value)
        )

FC_REFLECT_DERIVED( fbtc::wallet::account_data, (fbtc::blockchain::account_record),
		(is_my_account)
...
FC_REFLECT_DERIVED(fbtc::wallet::account_address_data, (fbtc::wallet::account_data),
	(owner_address)
	(active_address)
	)
FC_REFLECT( fbtc::wallet::master_key,
        (encrypted_key)
        (checksum)
        )

FC_REFLECT( fbtc::wallet::key_data,
        (account_address)
        (child_key_index)
        (public_key)
...
FC_REFLECT_ENUM( fbtc::wallet::contact_data::contact_type_enum,
        (account_name)
        (public_key)
        (address)
        (btc_address)
...
FC_REFLECT( fbtc::wallet::contact_data,
        (contact_type)
        (data)
        (label)
        (custom_data)
...
FC_REFLECT( fbtc::wallet::approval_data,
        (name)
        (approval)
        (custom_data)
        )

FC_REFLECT( fbtc::wallet::transaction_data,
        (record_id)
        (block_num)
        (is_virtual)
        (is_confirmed)
...
FC_REFLECT( fbtc::wallet::ledger_entry,
        (from_account)
        (to_account)
        (amount)
        (memo)
...
FC_REFLECT( fbtc::wallet::transaction_ledger_entry,
        (id)
        (block_num)
        (block_id)
        (timestamp)
...
FC_REFLECT_DERIVED( fbtc::wallet::pretty_transaction_experimental, (fbtc::wallet::transaction_ledger_entry),
        (inputs)
        (outputs)
        (balances)
        (notes)
...
FC_REFLECT( fbtc::wallet::setting,
        (name)
        (value)
        )

FC_REFLECT( fbtc::wallet::generic_wallet_record,
        (type)
        (data)
        )
...
namespace fc {
   ...
   template<typename T, fbtc::wallet::wallet_record_type_enum N>
   struct get_typename< fbtc::wallet::wallet_record<T,N> >
   {
   ...
   template<typename Type, fbtc::wallet::wallet_record_type_enum N>
   struct reflector<fbtc::wallet::wallet_record<Type,N>>
   {
      typedef fbtc::wallet::wallet_record<Type,N>  type;
      ...
      {
          {
            typedef decltype(((fbtc::wallet::base_record<N>*)nullptr)->wallet_record_index) member_type;
            visitor.TEMPLATE operator()<member_type,fbtc::wallet::base_record<N>,&fbtc::wallet::base_record<N>::wallet_record_index>( "index" );
          }
