...
BOOST_FUSION_ADAPT_STRUCT(fbtc::bitcoin::python_dict_type_t,
			  (std::vector<fbtc::bitcoin::python_dict_elem_type_t>, items))

BOOST_FUSION_ADAPT_STRUCT(fbtc::bitcoin::python_dict_array_type_t,
			  (std::vector<fbtc::bitcoin::python_dict_val_t>, items))
...
private:
  ...
  fbtc::bitcoin::python_dict_type_t accounts;
     ...
     for( auto &account : accounts.items )
     {
        if( account.first == "0" )
        {
           ...
           if( encryption )
           {
              ...
              {
                 FC_THROW_EXCEPTION( fbtc::wallet::invalid_password, "wrong password" );
              }
           }
...
public:
  electrumwallet( std::string path ):encryption(false),seed_version(""),masterkey("")
  {
     ...
     fbtc::bitcoin::parser<std::string::const_iterator> grammar;
     fbtc::bitcoin::python_dict_type_t dict;
     ...
     if( r && iter == end )
     {
        for( auto &item : dict.items )
        {
           ...
           {
              accounts.items = boost::get<fbtc::bitcoin::python_dict_type_t>( item.second ).items;
           }
        }
     }
     else
