...
namespace fbtc { namespace client { namespace detail {

vector<account_record> client_impl::blockchain_list_active_delegates( uint32_t first, uint32_t count )const
{
   if( first > 0 ) --first;
   FC_ASSERT( first < FBTC_BLOCKCHAIN_NUM_DELEGATES );
   FC_ASSERT( first + count <= FBTC_BLOCKCHAIN_NUM_DELEGATES );
   vector<account_id_type> all_delegate_ids = _chain_db->get_active_delegates();
   FC_ASSERT( all_delegate_ids.size() == FBTC_BLOCKCHAIN_NUM_DELEGATES );
...
vector<string> client_impl::blockchain_list_missing_block_delegates( uint32_t block_num )
{
   ...
   timestamp += FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
...
int32_t client_impl::blockchain_get_pending_transactions_count() const
{
	return _chain_db->get_pending_transactions_size();
}
...
oaccount_record detail::client_impl::blockchain_get_account( const string& account )const
{
   try
   {
      ...
      else if( account.substr( 0, string( FBTC_ADDRESS_PREFIX ).size() ) == FBTC_ADDRESS_PREFIX ) {
...
variant_object client_impl::blockchain_get_info()const
{
   ...
   info["name"]                                 = FBTC_BLOCKCHAIN_NAME;
   info["symbol"]                               = FBTC_BLOCKCHAIN_SYMBOL;
   info["address_prefix"]                       = FBTC_ADDRESS_PREFIX;
   info["db_version"]                           = FBTC_BLOCKCHAIN_DATABASE_VERSION;
   info["genesis_timestamp"]                    = _chain_db->get_genesis_timestamp();

   info["block_interval"]                       = FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
   info["delegate_num"]                         = FBTC_BLOCKCHAIN_NUM_DELEGATES;
   ...
   info["name_size_max"]                        = FBTC_BLOCKCHAIN_MAX_NAME_SIZE;
   info["memo_size_max"]                        = FBTC_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE;

   info["symbol_size_max"]                      = FBTC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE;
   info["symbol_size_min"]                      = FBTC_BLOCKCHAIN_MIN_SYMBOL_SIZE;
   info["asset_shares_max"]                     = FBTC_BLOCKCHAIN_MAX_SHARES;
   info["short_symbol_asset_reg_fee"]           = _chain_db->get_asset_registration_fee( FBTC_BLOCKCHAIN_MIN_SYMBOL_SIZE );
   info["long_symbol_asset_reg_fee"]            = _chain_db->get_asset_registration_fee( FBTC_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE );

   info["statistics_enabled"]                   = _chain_db->get_statistics_enabled();

   info["relay_fee"]                            = _chain_db->get_relay_fee();
   info["max_pending_queue_size"]               = FBTC_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
   info["max_trx_per_second"]                   = FBTC_BLOCKCHAIN_MAX_TRX_PER_SECOND;
...
{ try {
   ...
   return asset( _chain_db->calculate_defbtc( include_interest ).at( asset_id ), asset_id );
...
{ try {
   ...
   const share_type pay_per_block = FBTC_MAX_DELEGATE_PAY_PER_BLOCK * average_delegate_pay_rate / 100;
...
bool client_impl::blockchain_verify_signature(const string& signer, const fc::sha256& hash, const fc::ecc::compact_signature& signature) const
{
    ...
    catch (...)
    {
        try {
           ...
           // NB LHS of == operator is fbtc::blockchain::public_key_type and RHS is fc::ecc::public_key,
...
vector<fbtc::blockchain::string_status_record> client_impl::blockchain_list_markets()const
{
   const vector<pair<asset_id_type, asset_id_type>> pairs = _chain_db->get_market_pairs();

   vector<fbtc::blockchain::string_status_record> statuses;
...
vector<fbtc::blockchain::market_transaction> client_impl::blockchain_list_market_transactions( uint32_t block_num )const
{
   return _chain_db->get_market_transactions( block_num );
}

fbtc::blockchain::string_status_record client_impl::blockchain_market_status( const std::string& quote,
...
fbtc::blockchain::asset client_impl::blockchain_unclaimed_genesis() const
{
