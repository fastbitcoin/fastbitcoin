...
#include <fbtc/client/seed_nodes.hpp>
#include <fbtc/net/node.hpp>
#include <fbtc/rpc/rpc_client_api.hpp>
...
namespace fbtc { namespace client {
    ...
    struct rpc_server_config
    {
      ...
      fc::path         htdocs;
      map<string,fbtc::api::permissions> users;
    ...
    struct config
    {
        ...
        uint16_t            maximum_number_of_connections = FBTC_NET_DEFAULT_MAX_CONNECTIONS;
        bool                use_upnp = true;
        ...
        share_type          min_relay_fee = FBTC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
        string              wallet_callback_url;

        share_type          light_network_fee = FBTC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
        share_type          light_relay_fee = FBTC_BLOCKCHAIN_DEFAULT_RELAY_FEE;
        ...
        optional<string>    growl_fbtcshares_client_identifier;
    };
     ...
     *
     */
    class client : public fbtc::rpc_stubs::common_api_client,
                   public std::enable_shared_from_this<client>
    {
       public:
         ...
         fbtc::rpc::rpc_server_ptr   get_rpc_server()const;
         fbtc::net::node_ptr         get_node()const;
         fc::path                   get_data_dir()const;

         // returns true if the client is connected to the network
         bool                is_connected() const;
         fbtc::net::node_id_t get_node_id() const;
         ...
         bool handle_message(const fbtc::net::message&, bool sync_mode);
         void sync_status(uint32_t item_type, uint32_t item_count);

       protected:
         virtual fbtc::api::common_api* get_impl() const override;
...
extern const std::string FBTC_MESSAGE_MAGIC;


FC_REFLECT(fbtc::client::client_notification, (timestamp)(message)(signature) )
FC_REFLECT( fbtc::client::rpc_server_config, (enable)(enable_cache)(rpc_user)(rpc_password)(rpc_endpoint)(httpd_endpoint)(websocket_endpoint)
            (encrypted_rpc_endpoint)(encrypted_rpc_wif_key)(htdocs)(users)(whitelist) )
FC_REFLECT( fbtc::client::chain_server_config, (enabled)(listen_port) )
FC_REFLECT( fbtc::client::config,
        (logging)
        (ignore_console)
        (client_debug_name)
        (rpc)
        ...
        (growl_password)
        (growl_fbtcshares_client_identifier)
        (rpc)
    )

FC_API( fbtc::api::common_api,
        (about)
        (get_info)
      )
