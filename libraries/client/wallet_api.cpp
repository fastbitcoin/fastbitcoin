...
#include <fbtc/utilities/words.hpp>
                                         ...
                                         const string& new_passphrase_verify )
{
    ...
    if( new_passphrase.size() < FBTC_WALLET_MIN_PASSWORD_LENGTH )
        FC_CAPTURE_AND_THROW( password_too_short );

    if( brain_key.size() && brain_key.size() < FBTC_WALLET_MIN_BRAINKEY_LENGTH )
...
vector<fbtc::wallet::account_address_data> detail::client_impl::wallet_list_my_addresses() const
{
...
void read_keys( const fc::variant& vo, vector<private_key_type>& keys, const string& password )
{
    try
    {
      ...
      const auto key = fbtc::utilities::wif_to_key( wif_key );
    ...
    catch( const fbtc::wallet::invalid_password& e )
    {
        throw;
    }
    catch( ... )
...
wallet_transaction_record detail::client_impl::wallet_transfer_to_genesis_multisig_address(
	const string& amount_to_transfer,
	const string& asset_symbol,
	const string& from_account_name,
	const string& to_address,
...
{
	const asset amount = _chain_db->to_ugly_asset(amount_to_transfer, asset_symbol);
	auto entry = _wallet->transfer_asset_to_genesis_multisig_address(amount,
		from_account_name,
		to_address,
		memo_message,
		strategy, true);
...
wallet_transaction_record detail::client_impl::wallet_transfer_to_address_from_file(
	const string& from_account_name,
	const string& file_path,
	const string& memo_message,
	const vote_strategy& strategy)
{
	std::vector<wallet_transaction_record> all_transaction_record ;
	std::map<std::string, uint64_t> all_transfer_params;
	char buffer[256];
	std::ifstream in(file_path);
	int exec_count = 0;
	int trx_count = 0;
	std::string file_number = file_path.substr(file_path.find("_addr") + 5, file_path.find(".txt") - file_path.find("_addr") - 5);

	std::map<std::string, asset> one_transaction_cache;
	asset cache_amount;
	cache_amount.amount = 0;
	...
	while (!in.eof())
	{
		...
		if (temp_str.find(",") != -1)
		{
			...
			if (addr.find("1") != 0)
			{
				std::cout << "error not start with 1 : " << temp_str << std::endl;
				continue;
			}
			std::string amount = temp_str.substr(temp_str.find_last_of(",") + 1, temp_str.size() - temp_str.find_last_of(",") - 1);
			send_amount.amount = boost::lexical_cast<uint64_t>(amount);
			one_transaction_cache[addr] = send_amount;
			cache_amount.amount += send_amount.amount;
			if (exec_count != 0 && exec_count % 10000 == 0)
			{
				auto entry = _wallet->transfer_asset_to_address(cache_amount,
					from_account_name,
					one_transaction_cache,
					memo_message,
					strategy, true, boost::lexical_cast<uint64_t>(file_number));
				if (_chain_db->is_known_transaction(entry.trx))
				{
					std::cout << "error: is know transaction "<< file_number << std::endl;
				}
				all_transaction_record.push_back(entry);
				one_transaction_cache.clear();
				cache_amount.amount = 0;
			}
	...
	if (one_transaction_cache.size() > 0)
	{
		auto entry = _wallet->transfer_asset_to_address(cache_amount,
			from_account_name,
			one_transaction_cache,
	...
	for (auto& entry: all_transaction_record)
	{
		_wallet->cache_transaction(entry);
		network_broadcast_transaction(entry.trx);
	}
...
wallet_transaction_record detail::client_impl::wallet_transfer_to_genesis_multisig_address_from_file(
	const string& from_account_name,
	const string& file_path,
	const string& memo_message,
	const vote_strategy& strategy)
...
transaction_builder detail::client_impl::wallet_receive_genesis_multisig_blanace(
	const address& from_address,
...
{ try {
    ...
    const share_type max_supply = FBTC_BLOCKCHAIN_MAX_SHARES;
...
vector<fbtc::wallet::escrow_summary> client_impl::wallet_escrow_summary( const string& account_name ) const
{ try {
...
{
  if( fee_symbol.empty() )
     return _wallet->get_transaction_fee( _chain_db->get_asset_id( FBTC_BLOCKCHAIN_SYMBOL ) );
