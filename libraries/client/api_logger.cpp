...
#include <fbtc/client/api_logger.hpp>
...
void stream_api_logger::log_call_started ( uint64_t call_id, const fbtc::api::common_api* target, const fc::string& name, const fc::variants& args )
{
    fc::scoped_lock<fc::mutex> lock(this->output_mutex);
...
void stream_api_logger::log_call_finished( uint64_t call_id, const fbtc::api::common_api* target, const fc::string& name, const fc::variants& args, const fc::variant& result )
{
    fc::scoped_lock<fc::mutex> lock(this->output_mutex);
    std::string cname = target->debug_get_client_name();
