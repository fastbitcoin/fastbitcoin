...
{
   return fbtc::client::version_info();
}
...
variant_object client_impl::get_info()const
{
   ...
   const auto share_record                                      = _chain_db->get_asset_record( FBTC_BLOCKCHAIN_SYMBOL );
   if( share_record.valid() )
      info["blockchain_share_supply"]                           = share_record->current_supply;

   const auto blocks_left                                       = FBTC_BLOCKCHAIN_NUM_DELEGATES - (head_block_num % FBTC_BLOCKCHAIN_NUM_DELEGATES);
   ...
   if( head_block_num > 0 )
   {
      ...
      const auto next_round_timestamp                           = current_round_timestamp + (blocks_left * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
   ...
   info["client_version"]                                       = fbtc::client::version_info()["client_version"].as_string();
...
{
   FC_ASSERT( _self->get_rpc_server()->get_method_data(method_name).prerequisites == fbtc::api::no_prerequisites,
