...
#include <fbtc/client/build_info.hpp>
#include <fbtc/client/client.hpp>
#include <fbtc/client/client_impl.hpp>
#include <fbtc/client/messages.hpp>

#include <fbtc/db/level_map.hpp>
...
#include <fbtc/blockchain/transaction_evaluation_state.hpp>

#include <fbtc/net/chain_downloader.hpp>
#include <fbtc/net/exceptions.hpp>

#include <fbtc/api/common_api.hpp>
#include <fbtc/rpc/rpc_client.hpp>
#include <fbtc/rpc/rpc_server.hpp>
...
const string FBTC_MESSAGE_MAGIC = "Fbtc Signed Message:\n";
...
program_options::variables_map parse_option_variables(int argc, char** argv)
{
   ...
   option_config.add_options()
         ...
         ("growl-identifier", program_options::value<std::string>(), "A name displayed in growl messages to identify this fbtcshares_client instance")
   ...
   {
      std::cout << fc::json::to_pretty_string( fbtc::client::version_info() ) << "\n";
      exit(0);
   }
...
#ifdef FBTC_TEST_NETWORK
             dir_name += "-Test" + std::to_string( FBTC_TEST_NETWORK_VERSION );
#endif
             return dir_name;
         };

         datadir = fc::app_path() / get_os_specific_dir_name( FBTC_BLOCKCHAIN_NAME );

         if( !fc::exists( datadir ) )
         {
             const auto old_data_dir = fc::app_path() / get_os_specific_dir_name( "Fbtc X" );
                                       ...
                                       const program_options::variables_map& option_variables)
{
   if( option_variables.count("server") || option_variables.count("daemon") || cfg.rpc.enable )
   {
      ...
      // fbtc::rpc::rpc_server::config rpc_config(cfg.rpc);
...
void client_impl::start_delegate_loop()
{
    ...
    if (!_time_discontinuity_connection.connected())
        _time_discontinuity_connection = fbtc::blockchain::time_discontinuity_signal.connect([=](){ reschedule_delegate_loop(); });
...
void client_impl::delegate_loop()
{
   ...
   {
      set_target_connections( FBTC_NET_DELEGATE_DESIRED_CONNECTIONS );
   ...
   if( next_block_time.valid() )
   {
      ...
      if( *next_block_time <= now )
      {
         try
         {
            ...
            FC_ASSERT( (now - *next_block_time) < fc::seconds( FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC ),
...
void client_impl::start_rebroadcast_pending_loop()
{
   if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
      ...
      fc::time_point::now() + fc::seconds((int64_t)(FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
      "rebroadcast_pending" );
}
      ...
      try
      {
         ...
         if( fork_data && fork_data->is_known )
         {
            if (sync_mode && !fork_data->is_linked)
               FC_THROW_EXCEPTION(fbtc::blockchain::unlinkable_block,
         ...
         else
         {
            ...
            if (sync_mode && !result.is_linked)
               FC_THROW_EXCEPTION(fbtc::blockchain::unlinkable_block, "The blockchain accepted this block, but it isn't linked");
...
///////////////////////////////////////////////////////
bool client_impl::has_item(const fbtc::net::item_id& id)
{
...
bool client_impl::handle_message(const fbtc::net::message& message_to_handle, bool sync_mode)
{
   try
   {
      ...
      case block_message_type:
      {
         ...
         fbtc::blockchain::block_id_type old_head_block = _chain_db->get_head_block_id();
   ...
   catch (const fbtc::blockchain::insufficient_relay_fee& original_exception)
   {
      FC_THROW_EXCEPTION(fbtc::net::insufficient_relay_fee, "Insufficient relay fee; do not propagate!",
                         ("original_exception", original_exception.to_detail_string()));
   }
   catch (const fbtc::blockchain::block_older_than_undo_history& original_exception)
   {
      FC_THROW_EXCEPTION(fbtc::net::block_older_than_undo_history, "Block is older than undo history, stop fetching blocks!",
                         ("original_exception", original_exception.to_detail_string()));
   }
}
...
/**
  ...
  */
std::vector<fbtc::net::item_hash_t> client_impl::get_item_ids(uint32_t item_type,
                                                             const std::vector<fbtc::net::item_hash_t>& blockchain_synopsis,
                                                             uint32_t& remaining_item_count,
                                                             uint32_t limit /* = 2000 */)
{
   // limit = 20; // for testing
   FC_ASSERT(item_type == fbtc::client::block_message_type);
   ...
   fbtc::net::item_hash_t last_seen_block_hash;
   for (const fbtc::net::item_hash_t& item_hash : boost::adaptors::reverse(blockchain_synopsis))
   {
      try
      {
   ...
   std::vector<fbtc::net::item_hash_t> hashes_to_return;
...
fbtc::net::message client_impl::get_item(const fbtc::net::item_id& id)
{
   if (id.item_type == block_message_type)
   {
      //   uint32_t block_number = _chain_db->get_block_num(id.item_hash);
      fbtc::client::block_message block_message_to_send(_chain_db->get_block(id.item_hash));
...
uint32_t client_impl::get_block_number(const fbtc::net::item_hash_t& block_id)
{
   return _chain_db->get_block_num(block_id);
}

fc::time_point_sec client_impl::get_block_time(const fbtc::net::item_hash_t& block_id)
{
   if (block_id == fbtc::net::item_hash_t())
   {
...
fc::time_point_sec client_impl::get_blockchain_now()
{
   ...
   return fbtc::blockchain::nonblocking_now();
}

fbtc::net::item_hash_t client_impl::get_head_block_id() const
{
...
{
   return fbtc::blockchain::estimate_last_known_fork_from_git_revision_timestamp(unix_timestamp);
}
...
void client_impl::blocks_too_old_monitor_task()
{
   ...
   if (_chain_db->get_head_block().timestamp < fbtc::blockchain::now() - fc::seconds(FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
       !_sync_mode &&
   ...
   if (!_blocks_too_old_monitor_done.canceled())
      ...
      fc::time_point::now() + fc::seconds(FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
      "block_monitor_task");
}
...
client::client(const std::string& user_agent,
               fbtc::net::simulated_network_ptr network_to_connect_to)
   : my( new detail::client_impl(this, user_agent) )
{
...
{ try {
    ...
    my->_wallet = std::make_shared<fbtc::wallet::wallet>( my->_chain_db, my->_config.wallet_enabled );
    ...
    if( !my->_p2p_node )
       my->_p2p_node = std::make_shared<fbtc::net::node>( my->_user_agent );
...
fbtc::rpc::rpc_server_ptr client::get_rpc_server()const { return my->_rpc_server; }
fbtc::net::node_ptr client::get_node()const { return my->_p2p_node; }

fc::variant_object version_info()
{
   string client_version( fbtc::utilities::git_revision_description );
   const size_t pos = client_version.find( '/' );
   if( pos != string::npos && client_version.size() > pos )
       client_version = client_version.substr( pos + 1 );

#ifdef FBTC_TEST_NETWORK
   client_version += "-testnet-" + std::to_string( FBTC_TEST_NETWORK_VERSION );
#endif

   fc::mutable_variant_object info;
   info["blockchain_name"]          = FBTC_BLOCKCHAIN_NAME;
   info["blockchain_description"]   = FBTC_BLOCKCHAIN_DESCRIPTION;
   info["client_version"]           = client_version;
   info["fbtcshares_revision"]       = fbtc::utilities::git_revision_sha;
   info["fbtcshares_revision_age"]   = fc::get_approximate_relative_time_string( fc::time_point_sec( fbtc::utilities::git_revision_unix_timestamp ) );
...
#if defined(FBTC_CLIENT_JENKINS_BUILD_NUMBER)
   info["jenkins_build_number"] = FBTC_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(FBTC_CLIENT_JENKINS_BUILD_URL)
   info["jenkins_build_url"] = FBTC_CLIENT_JENKINS_BUILD_URL;
#endif
...
void client::start_networking(std::function<void()> network_started_callback)
{
   ...
   {
      fbtc::net::chain_downloader* chain_downloader = new fbtc::net::chain_downloader();
...
void client::configure_from_command_line(int argc, char** argv)
{
   ...
   {
      my->_cli = new fbtc::cli::cli( this, nullptr, &std::cout );
      return;
   }
   // parse command-line options
   ...
   if( option_variables.count("debug-stop") )
   {
      ...
      // intend to attach with GDB (e.g. fbtctests recommended GDB usage)
      //
   ...
   fc::path datadir = fbtc::client::get_data_dir(option_variables);
   if( !fc::exists( datadir ) )
   {
   ...
   // else we use the default set in fbtc::net::node
   ...
   else //we will accept input from the console
   {
      ...
      if( option_variables.count("log-commands") <= 0)
      {
         ...
         my->_cli = new fbtc::cli::cli( this, my->_command_script_holder.get(), &std::cout );
      }
      else
      {
         ...
         my->_cli = new fbtc::cli::cli( this, my->_command_script_holder.get(), my->_tee_stream.get() );
   ...
   if( option_variables["upnp"].as<bool>() )
   {
      ...
      my->_upnp_service = std::unique_ptr<fbtc::net::upnp_service>(new fbtc::net::upnp_service);
   ...
   if (option_variables.count("growl") || my->_config.growl_notify_endpoint)
   {
      ...
      std::string fbtc_instance_identifier = "Fbtc";
      if (option_variables.count("growl-identifier"))
         fbtc_instance_identifier = option_variables["growl-identifier"].as<std::string>();
      else if (my->_config.growl_fbtcshares_client_identifier)
         fbtc_instance_identifier = *my->_config.growl_fbtcshares_client_identifier;
      my->_notifier = std::make_shared<fbtc_gntp_notifier>(host_to_notify, port_to_notify, fbtc_instance_identifier, growl_password);
   ...
   {
      my->_chain_server = std::unique_ptr<fbtc::net::chain_server>(
               new fbtc::net::chain_server(my->_chain_db,
...
fbtc::net::node_id_t client::get_node_id() const
{
...
void client::add_node( const string& remote_endpoint )
{
   ...
   catch (const fbtc::net::already_connected_to_requested_peer&)
   {
   }
}
...
{
   fbtc::net::item_id head_item_id;
   head_item_id.item_type = fbtc::client::block_message_type;
   uint32_t last_block_num = my->_chain_db->get_head_block_num();
   if (last_block_num == (uint32_t)-1)
      head_item_id.item_hash = fbtc::net::item_hash_t();
   else
      head_item_id.item_hash = my->_chain_db->get_head_block_id();
   my->_p2p_node->sync_from(head_item_id, fbtc::blockchain::get_list_of_fork_block_numbers());
...
bool client::handle_message(const fbtc::net::message& message, bool sync_mode)
{
...
fbtc::api::common_api* client::get_impl() const
{
   return my.get();
}
