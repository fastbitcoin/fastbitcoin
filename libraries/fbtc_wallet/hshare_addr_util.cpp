...
namespace fbtc
{
	...
	std::string getHshareAddr(std::vector<unsigned char> hshare_pubkey)
	{
		...
		fbtc::blockchain::pts_address ptsaddr(pubkey, true, 40);
	...
	bool ReadKeyValue(CDataStream& ssKey, CDataStream& ssValue, std::string& strType, std::string& strErr, std::vector<std::string>& vwif, std::set<std::string>& vKeyPool)
	{
		try
		{
...
//				printf("address : %s ", getHshareAddr(tempvchPubKey).c_str());
				...
				vwif.emplace_back(fbtc::utilities::key_to_wif(pkey));

//				printf("private : %s ", hsrcore::utilities::key_to_wif(pkey).c_str());
				auto ttee = pkey.get_public_key();
				fbtc::blockchain::pts_address ptsaddr(pkey.get_public_key(), true, 40);
				std::string str = ptsaddr;
//				printf("my address : %s \n", str.c_str());
	...
	DBErrors getUnencryptKeys( std::vector<std::string>& vwif, const std::string& infile)
	{
		...
		{
			auto pkey = fbtc::utilities::wif_to_key(temp_str);
			if (pkey)
			{
	...
	bool ReadCryptKeyValue(CDataStream& ssKey, CDataStream& ssValue, std::string& strType, std::string& strErr,
		                   std::vector<CMasterKey>& vmkey, std::vector<std::vector<unsigned char>>& vpubkey, std::vector<std::vector<unsigned char>>& vcryptsecret)
	{
		try
		{
	...
	DBErrors dumpCryptKeys(std::vector<CMasterKey>& vmkey, std::vector<std::vector<unsigned char>>& vpubkey, std::vector<vector<unsigned char>>& vsecret, const std::string& infile)
	{
...
/*	int testEncyptLen(std::vector<unsigned char>& src, std::vector<unsigned char>& des)
	...
	void getHsharePrivKey(std::vector<unsigned char>& hshare_privkey, const fbtc::CKeyingMaterial& vchSecret)
	{
	...
	DBErrors getEncryptKeys(std::vector<std::string>& vwif, const std::string& passwd, const std::string& infile)
	{
		...
		std::vector<fbtc::CMasterKey> vmkey;
		std::vector<vector<unsigned char>> vpubkey;
		std::vector<vector<unsigned char>> vcryptsecret;
		fbtc::dumpCryptKeys(vmkey, vpubkey, vcryptsecret, infile);

		for (int i = 0; i < vpubkey.size(); ++i)
		{
//			printf("pubkey addr: %s\n", hshare::getHshareAddr(vpubkey[i]).c_str());

			auto h = fbtc::Hash(vpubkey[i].begin(), vpubkey[i].end());
			
			if (!fbtc::setKeyFromPassphrase(passwd, vmkey[0].vchSalt, vmkey[0].nDeriveIterations, vmkey[0].nDerivationMethod))
			{
			...
			fbtc::CKeyingMaterial vMasterKey;
			if (!fbtc::descrpt(vmkey[0].vchCryptedKey, vMasterKey))
			{
			...
			fbtc::CKeyingMaterial vchSecret;

			fbtc::decryptSecret(vMasterKey, vcryptsecret[i], h, vchSecret);
			...
			fbtc::getHsharePrivKey(privKey, vchSecret);

			fc::ecc::private_key pkey;

			pkey = fbtc::changeHsharePrivkey(privKey, false);
			vwif.emplace_back(fbtc::utilities::key_to_wif(pkey));
