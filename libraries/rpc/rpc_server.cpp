...
#include <fbtc/rpc/exceptions.hpp>
...
#include <fbtc/utilities/padding_ostream.hpp>
...
#include <fbtc/api/reflect_api.hpp>
...
namespace fbtc { namespace rpc {
  ...
  namespace detail
  {
    class rpc_server_impl : public fbtc::rpc_stubs::common_api_rpc_server
    {
       public:
         rpc_server_config                                 _config;
         fbtc::client::client*                              _client;
         ...
         rpc_server_impl(fbtc::client::client* client) :
           _client(client),
         ...
         virtual fbtc::api::common_api* get_client() const override;
         ...
         virtual void store_method_metadata(const fbtc::api::method_data& method_metadata)override;

         std::string help(const std::string& command_name) const;

         std::string make_short_description(const fbtc::api::method_data& method_data, bool show_decription = true) const
         {
           std::string help_string;
           std::stringstream short_description;
           short_description << std::setw(100) << std::left;
           help_string = method_data.name + " ";
           for (const fbtc::api::parameter_data& parameter : method_data.parameters)
           {
             if (parameter.classification == fbtc::api::required_positional)
               help_string += std::string("<") + parameter.name + std::string("> ");
             else if (parameter.classification == fbtc::api::optional_named)
               help_string += std::string("{") + parameter.name + std::string("} ");
             else if (parameter.classification == fbtc::api::required_positional_hidden)
               continue;
             else
               help_string += std::string("[") + parameter.name + std::string("] ");
           }
        ...
        void add_content_type_header(const fc::string& path, const fc::http::server::response& s ) {
              ...
              //     dlog( "CHECKING AUTH" );
               ...
               //    ilog( "\n\n\n  username: ${user}  password ${password}", ("path",r.path)("user",username)("password", password));
                ...
                //      elog( "\n\n\n Unauthorized ${path}, username: ${user}  password ${password}", ("path",r.path)("user",username)("password", password));
                      s.add_header( "WWW-Authenticate", "Basic realm=\"fbtc wallet\"" );
         ...
         fc::http::reply::status_code handle_http_rpc(const fc::http::request& r, const fc::http::server::response& s )
         {
                ...
                try {
                   ...
                   if( fc::time_point(fbtc::blockchain::now()) - _last_cache_clear_time  > fc::seconds(FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC) )
                   {
                      _last_cache_clear_time = fbtc::blockchain::now();
                      _call_cache.clear();
                   }
         ...
         void encrypted_accept_loop(const fbtc::blockchain::private_key_type& server_key)
         {
        ...
        fc::variant dispatch_method_from_json_connection(fc::rpc::json_connection* con,
                                                         const fbtc::api::method_data& method_data,
                                                         const fc::variants& arguments)
        {
          // ilog( "arguments: ${params}", ("params",arguments) );
          if ((method_data.prerequisites & fbtc::api::json_authenticated) &&
        ...
        fc::variant dispatch_authenticated_method(const fbtc::api::method_data& method_data,
                                                  const fc::variants& arguments_from_caller)
        {
          ...
          if (method_data.prerequisites & fbtc::api::wallet_open)
            verify_wallet_is_open();
          if (method_data.prerequisites & fbtc::api::wallet_unlocked)
            verify_wallet_is_unlocked();
          if (method_data.prerequisites & fbtc::api::connected_to_network)
          ...
          {
            if (parameter.classification == fbtc::api::required_positional
                || parameter.classification == fbtc::api::required_positional_hidden)
            {
            ...
            else if (parameter.classification == fbtc::api::optional_positional)
            {
    ...
    fbtc::api::common_api* rpc_server_impl::get_client() const
    {
      return _client;
    }
    ...
    void rpc_server_impl::store_method_metadata(const fbtc::api::method_data& method_metadata)
    {
    ...
    std::string rpc_server_impl::help(const std::string& command_name) const
    {
      ...
      { //display detailed description of requested command
        ...
        if (alias_iter != _alias_map.end())
        {
          ...
          const fbtc::api::method_data& method_data = method_iter->second;
          help_string += "Usage:\n";
  ...
  rpc_server::rpc_server(fbtc::client::client* client) :
  ...
  bool rpc_server::configure_websockets( const rpc_server_config& cfg )
  {
    ...
    try
    {
      ...
      my->_websocket_server->on_connection([&]( const fc::http::websocket_connection_ptr& c ){
               ...
               auto login = std::make_shared<fbtc::api::login_api>( cfg.users, my->_client );
               wsc->register_api(fc::api<fbtc::api::login_api>(login));
  ...
  const fbtc::api::method_data& rpc_server::get_method_data(const std::string& method_name)
  {
  ...
  std::vector<fbtc::api::method_data> rpc_server::get_all_method_data() const
  {
    std::vector<fbtc::api::method_data> result;
    for (const detail::rpc_server_impl::method_map_type::value_type& value : my->_method_map)
      result.emplace_back(value.second);
    return result;
  }

  void rpc_server::register_method(fbtc::api::method_data data)
  {
