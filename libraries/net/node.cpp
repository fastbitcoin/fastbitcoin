...
#include <fbtc/net/peer_database.hpp>
#include <fbtc/net/peer_connection.hpp>
#include <fbtc/net/stcp_socket.hpp>
#include <fbtc/net/config.hpp>
#include <fbtc/net/exceptions.hpp>
...
} } } // end namespace fbtc::net::detail
FC_REFLECT(fbtc::net::detail::node_configuration, (listen_endpoint)
...
namespace fbtc { namespace net { namespace detail {
    ...
    struct prioritized_item_id
    {
      ...
      {
        static_assert(fbtc::client::block_message_type > fbtc::client::trx_message_type,
...
#undef DECLARE_ACCUMULATOR
    ...
    public:
      ...
      std::vector<item_hash_t> get_blockchain_synopsis(uint32_t item_type,
                                                       const fbtc::net::item_hash_t& reference_point = fbtc::net::item_hash_t(),
...
#define POTENTIAL_PEER_DATABASE_FILENAME "peers.leveldb"
      ...
      typedef std::unordered_map<fbtc::blockchain::block_id_type, fc::time_point> active_sync_requests_map;

      active_sync_requests_map              _active_sync_requests; /// list of sync blocks we've asked for from peers but have not yet received
      std::list<fbtc::client::block_message> _new_received_sync_items; /// list of sync blocks we've just received but haven't yet tried to process
      std::list<fbtc::client::block_message> _received_sync_items; /// list of sync blocks we've received, but can't yet process because we are still missing blocks that come earlier in the chain
      // @}
...
//#define USE_PEERS_TO_DELETE_MUTEX 1
      ...
      void send_sync_block_to_node_delegate(const fbtc::client::block_message& block_message_to_send);
      void process_backlog_of_sync_blocks();
      void trigger_process_backlog_of_sync_blocks();
      void process_block_during_sync(peer_connection* originating_peer, const fbtc::client::block_message& block_message, const message_hash_type& message_hash);
      void process_block_during_normal_operation(peer_connection* originating_peer, const fbtc::client::block_message& block_message, const message_hash_type& message_hash);
      ...
      message_propagation_data   get_transaction_propagation_data( const fbtc::blockchain::transaction_id_type& transaction_id );
      message_propagation_data   get_block_propagation_data( const fbtc::blockchain::block_id_type& block_id );
...
#define MAXIMUM_NUMBER_OF_BLOCKS_TO_PREFETCH (10 * MAXIMUM_NUMBER_OF_BLOCKS_TO_HANDLE_AT_ONE_TIME)
    ...
    node_impl::node_impl(const std::string& user_agent) :
      ...
      _desired_number_of_connections(FBTC_NET_DEFAULT_DESIRED_CONNECTIONS),
      _maximum_number_of_connections(FBTC_NET_DEFAULT_MAX_CONNECTIONS),
      _peer_connection_retry_timeout(FBTC_NET_DEFAULT_PEER_CONNECTION_RETRY_TIME),
      _peer_inactivity_timeout(FBTC_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT),
      ...
      _maximum_blocks_per_peer_during_syncing(FBTC_NET_MAX_BLOCKS_PER_PEER_DURING_SYNCING)
    {
    ...
    void node_impl::p2p_network_connect_loop()
    {
      ...
      while (!_p2p_network_connect_loop_done.canceled())
      {
        try
        {
          ...
          try
          {
            _retrigger_connect_loop_promise = fc::promise<void>::ptr( new fc::promise<void>("fbtc::net::retrigger_connect_loop") );
            if( is_wanting_new_connections() || !_add_once_node_list.empty() )
            {
              ...
              _retrigger_connect_loop_promise->wait_until( fc::time_point::now() + fc::seconds(FBTC_PEER_DATABASE_RETRY_DELAY ) );
            }
            else
            {
    ...
    bool node_impl::have_already_received_sync_item( const item_hash_t& item_hash )
    {
      ...
      return std::find_if(_received_sync_items.begin(), _received_sync_items.end(),
                          [&item_hash]( const fbtc::client::block_message& message ) { return message.block_id == item_hash; } ) != _received_sync_items.end() ||
             std::find_if(_new_received_sync_items.begin(), _new_received_sync_items.end(),
                          [&item_hash]( const fbtc::client::block_message& message ) { return message.block_id == item_hash; } ) != _new_received_sync_items.end();                          ;
    }
    ...
    void node_impl::request_sync_item_from_peer( const peer_connection_ptr& peer, const item_hash_t& item_to_request )
    {
      ...
      item_id item_id_to_request( fbtc::client::block_message_type, item_to_request );
    ...
    void node_impl::request_sync_items_from_peer( const peer_connection_ptr& peer, const std::vector<item_hash_t>& items_to_request )
    {
      ...
      peer->send_message(fetch_items_message(fbtc::client::block_message_type, items_to_request));
    }
    ...
    void node_impl::fetch_sync_items_loop()
    {
      ...
      while( !_fetch_sync_items_loop_done.canceled() )
      {
        ...
        if( !_sync_items_to_fetch_updated )
        {
          ...
          _retrigger_fetch_sync_items_loop_promise = fc::promise<void>::ptr( new fc::promise<void>("fbtc::net::retrigger_fetch_sync_items_loop") );
      ...
      while (!_fetch_item_loop_done.canceled())
      {
        ...
        for (auto iter = _items_to_fetch.begin(); iter != _items_to_fetch.end();)
        {
          ...
          for (const peer_connection_ptr& peer : _active_connections)
          {
            ...
            {
              if (peer->is_transaction_fetching_inhibited() && iter->item.item_type == fbtc::client::trx_message_type)
        ...
        {
          _retrigger_fetch_item_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("fbtc::net::retrigger_fetch_item_loop"));
    ...
    void node_impl::advertise_inventory_loop()
    {
      ...
      while (!_advertise_inventory_loop_done.canceled())
      {
        ...
        {
          _retrigger_advertise_inventory_loop_promise = fc::promise<void>::ptr(new fc::promise<void>("fbtc::net::retrigger_advertise_inventory_loop"));
    ...
    void node_impl::terminate_inactive_connections_loop()
    {
      ...
      uint32_t active_disconnect_timeout = std::max<uint32_t>(5 * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 2, 300);
      uint32_t active_send_keepalive_timeount = std::max<uint32_t>(active_disconnect_timeout / 2, 110);
      uint32_t active_ignored_request_timeount = std::max<uint32_t>(FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC / 4, 100);
      ...
      fc::time_point closing_disconnect_threshold = fc::time_point::now() - fc::seconds(FBTC_NET_PEER_DISCONNECT_TIMEOUT);
      for( const peer_connection_ptr& closing_peer : _closing_connections )
        ...
        {
          // we asked this peer to close their connectoin to us at least FBTC_NET_PEER_DISCONNECT_TIMEOUT
      ...
      if (!_node_is_shutting_down && !_terminate_inactive_connections_loop_done.canceled())
         _terminate_inactive_connections_loop_done = fc::schedule( [this](){ terminate_inactive_connections_loop(); },
                                                                   fc::time_point::now() + fc::seconds(FBTC_NET_PEER_HANDSHAKE_INACTIVITY_TIMEOUT / 2),
    ...
    bool node_impl::merge_address_info_with_potential_peer_database(const std::vector<address_info> addresses)
    {
      ...
      {
        if (address.firewalled == fbtc::net::firewalled_state::not_firewalled)
        {
    ...
    void node_impl::on_message( peer_connection* originating_peer, const message& received_message )
    {
      ...
      dlog("handling message ${type} ${hash} size ${size} from peer ${endpoint}",
           ("type", fbtc::net::core_message_type_enum(received_message.msg_type))("hash", message_hash)
      ...
      case core_message_type_enum::closing_connection_message_type:
        ...
        break;
      case fbtc::client::message_type_enum::block_message_type:
    ...
    fc::variant_object node_impl::generate_hello_user_data()
    {
      ...
      user_data["fbtcshares_git_revision_sha"] = fbtc::utilities::git_revision_sha;
      user_data["fbtcshares_git_revision_unix_timestamp"] = fbtc::utilities::git_revision_unix_timestamp;
    ...
    void node_impl::parse_hello_user_data_for_peer(peer_connection* originating_peer, const fc::variant_object& user_data)
    {
      ...
      if (user_data.contains("fbtcshares_git_revision_sha"))
        originating_peer->fbtcshares_git_revision_sha = user_data["fbtcshares_git_revision_sha"].as_string();
      if (user_data.contains("fbtcshares_git_revision_unix_timestamp"))
        originating_peer->fbtcshares_git_revision_unix_timestamp = fc::time_point_sec(user_data["fbtcshares_git_revision_unix_timestamp"].as<uint32_t>());
      ...
      // EMF: Fix for the main blockchain (fbtcshares & master branch in git) only:
      // the FBTC_FORK_TO_UNIX_TIME_LIST wasn't maintained for a few versions (0.4.23 - 0.4.28)
    ...
    void node_impl::on_hello_message( peer_connection* originating_peer, const hello_message& hello_message_received )
    {
      ...
      if (originating_peer->last_known_fork_block_number == 0 &&
          originating_peer->fbtcshares_git_revision_unix_timestamp)
      {
        uint32_t unix_timestamp = originating_peer->fbtcshares_git_revision_unix_timestamp->sec_since_epoch();
        ...
        if (originating_peer->last_known_fork_block_number != 0)
        {
          ...
          if (next_fork_block_number != 0)
          {
            ...
            if (next_fork_block_number < head_block_num)
            {
              ...
              ulog("Rejecting connection from peer because their version is too old.  Their version date: ${date}", ("date", originating_peer->fbtcshares_git_revision_unix_timestamp));
#endif
    ...
    void node_impl::on_address_message(peer_connection* originating_peer, const address_message& address_message_received)
    {
      ...
      std::vector<fbtc::net::address_info> updated_addresses = address_message_received.addresses;
                                                             ...
                                                             const blockchain_item_ids_inventory_message& blockchain_item_ids_inventory_message_received )
    {
      ...
      if( originating_peer->item_ids_requested_from_peer )
      {
        ...
        fc::time_point_sec minimum_time_of_last_offered_block =
            ...
            originating_peer->number_of_unfetched_item_ids * FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
        if (minimum_time_of_last_offered_block > _delegate->get_blockchain_now() + FBTC_NET_FUTURE_SYNC_BLOCKS_GRACE_PERIOD_SEC)
        {
        ...
        if (blockchain_item_ids_inventory_message_received.total_remaining_item_count != 0)
        {
          ...
          if (originating_peer->ids_of_items_to_get.size() > FBTC_NET_MIN_BLOCK_IDS_TO_PREFETCH)
          {
    ...
    void node_impl::on_fetch_items_message(peer_connection* originating_peer, const fetch_items_message& fetch_items_message_received)
    {
      ...
      {
        fbtc::client::block_message block = last_block_message_sent->as<fbtc::client::block_message>();
      ...
      for (const message& reply : reply_messages)
      {
        if (reply.msg_type == block_message_type)
          originating_peer->send_item(item_id(block_message_type, reply.as<fbtc::client::block_message>().block_id));
        else
          originating_peer->send_message(reply);
      }
    }
    ...
    void node_impl::on_item_ids_inventory_message(peer_connection* originating_peer, const item_ids_inventory_message& item_ids_inventory_message_received)
    {
      ...
      for( const item_hash_t& item_hash : item_ids_inventory_message_received.item_hashes_available )
      {
        ...
        if (!we_advertised_this_item_to_a_peer)
        {
          ...
          if ((item_ids_inventory_message_received.item_type == fbtc::client::trx_message_type &&
    ...
    void node_impl::send_sync_block_to_node_delegate(const fbtc::client::block_message& block_message_to_send)
    {
...
#ifdef ENABLE_DEBUG_ULOGS
                ulog("Disconnecting from peer during sync because their version is too old.  Their version date: ${date}", ("date", peer->fbtcshares_git_revision_unix_timestamp));
#endif
      ...
      do
      {
             ...
             ++received_block_iter)
        {
          ...
          if (potential_first_block)
          {
            ...
            {
              fbtc::client::block_message block_message_to_process = *received_block_iter;
    ...
    void node_impl::process_block_during_sync( peer_connection* originating_peer,
                                               const fbtc::client::block_message& block_message_to_process, const message_hash_type& message_hash )
    {
      VERIFY_CORRECT_THREAD();
    ...
    void node_impl::process_block_during_normal_operation( peer_connection* originating_peer,
                                                           const fbtc::client::block_message& block_message_to_process,
                                                           const message_hash_type& message_hash )
    {
      ...
      try
      {
        ...
        item_id block_message_item_id(fbtc::client::message_type_enum::block_message_type, message_hash);
...
#ifdef ENABLE_DEBUG_ULOGS
                ulog("Disconnecting from peer because their version is too old.  Their version date: ${date}", ("date", peer->fbtcshares_git_revision_unix_timestamp));
#endif
              }
            }
          }
                                          ...
                                          const message_hash_type& message_hash)
    {
      ...
      fbtc::client::block_message block_message_to_process(message_to_process.as<fbtc::client::block_message>());
      auto item_iter = originating_peer->items_requested_from_peer.find(item_id(fbtc::client::block_message_type, message_hash));
      ...
      else
      {
        ...
        auto sync_item_iter = originating_peer->sync_items_requested_from_peer.find(item_id(fbtc::client::block_message_type,
        ...
        if (sync_item_iter != originating_peer->sync_items_requested_from_peer.end())
        {
          ...
          if (originating_peer->idle())
          {
            ...
            if (originating_peer->number_of_unfetched_item_ids > 0 &&
                originating_peer->ids_of_items_to_get.size() < FBTC_NET_MIN_BLOCK_IDS_TO_PREFETCH)
              fetch_next_batch_of_item_ids_from_peer(originating_peer);
            else
              trigger_fetch_sync_items_loop();
          }
      ...
      fc::exception detailed_error(FC_LOG_MESSAGE(error, "You sent me a block that I didn't ask for, block_id: ${block_id}",
                                                  ("block_id", block_message_to_process.block_id)
                                                  ("fbtcshares_git_revision_sha", originating_peer->fbtcshares_git_revision_sha)
                                                  ("fbtcshares_git_revision_unix_timestamp", originating_peer->fbtcshares_git_revision_unix_timestamp)
                                                               ...
                                                               const get_current_connections_request_message& get_current_connections_request_message_received)
    {
      ...
      for (const peer_connection_ptr& peer : _active_connections)
      {
        ...
        fc::mutable_variant_object user_data;
        if (peer->fbtcshares_git_revision_sha)
          user_data["fbtcshares_git_revision_sha"] = *peer->fbtcshares_git_revision_sha;
        if (peer->fbtcshares_git_revision_unix_timestamp)
          user_data["fbtcshares_git_revision_unix_timestamp"] = *peer->fbtcshares_git_revision_unix_timestamp;
                                              ...
                                              const message& message_to_process, const message_hash_type& message_hash )
    {
      ...
      else
      {
        ...
        catch ( const insufficient_relay_fee& )
        {
          ...
          assert(message_to_process.msg_type == fbtc::client::trx_message_type); // we only support throttling transactions.
          if (message_to_process.msg_type == fbtc::client::trx_message_type)
            originating_peer->transaction_fetching_inhibited_until = fc::time_point::now() + fc::seconds(FBTC_NET_INSUFFICIENT_RELAY_FEE_PENALTY_SEC);
          return;
        }
        catch ( const fc::canceled_exception& )
        {
...
#ifdef FBTC_TEST_NETWORK
        uint32_t port = FBTC_NET_TEST_P2P_PORT + FBTC_TEST_NETWORK_VERSION;
#else
        uint32_t port = FBTC_NET_DEFAULT_P2P_PORT;
#endif
    ...
    std::vector<peer_status> node_impl::get_connected_peers() const
    {
      ...
      for (const peer_connection_ptr& peer : _active_connections)
      {
        ...
        if (peer->fbtcshares_git_revision_sha)
        {
          std::string revision_string = *peer->fbtcshares_git_revision_sha;
          if (*peer->fbtcshares_git_revision_sha == fbtc::utilities::git_revision_sha)
            revision_string += " (same as ours)";
          else
            revision_string += " (different from ours)";
          peer_details["fbtcshares_git_revision_sha"] = revision_string;

        }
        if (peer->fbtcshares_git_revision_unix_timestamp)
        {
          peer_details["fbtcshares_git_revision_unix_timestamp"] = *peer->fbtcshares_git_revision_unix_timestamp;
          std::string age_string = fc::get_approximate_relative_time_string(*peer->fbtcshares_git_revision_unix_timestamp);
          if (*peer->fbtcshares_git_revision_unix_timestamp == fc::time_point_sec(fbtc::utilities::git_revision_unix_timestamp))
            age_string += " (same as ours)";
          else if (*peer->fbtcshares_git_revision_unix_timestamp > fc::time_point_sec(fbtc::utilities::git_revision_unix_timestamp))
            age_string += " (newer than ours)";
          else
            age_string += " (older than ours)";
          peer_details["fbtcshares_git_revision_age"] = age_string;
        }
    ...
    void node_impl::broadcast( const message& item_to_broadcast, const message_propagation_data& propagation_data )
    {
      ...
      if( item_to_broadcast.msg_type == fbtc::client::block_message_type )
      {
        fbtc::client::block_message block_message_to_broadcast = item_to_broadcast.as<fbtc::client::block_message>();
      ...
      else if( item_to_broadcast.msg_type == fbtc::client::trx_message_type )
      {
        fbtc::client::trx_message transaction_message_to_broadcast = item_to_broadcast.as<fbtc::client::trx_message>();
    ...
    message_propagation_data node_impl::get_transaction_propagation_data( const fbtc::blockchain::transaction_id_type& transaction_id )
    {
      VERIFY_CORRECT_THREAD();
      return _message_cache.get_message_propagation_data( transaction_id );
    }

    message_propagation_data node_impl::get_block_propagation_data( const fbtc::blockchain::block_id_type& block_id )
    {
      VERIFY_CORRECT_THREAD();
...
# define INVOKE_IN_IMPL(method_name, ...) \
  ...
  message_propagation_data node::get_transaction_propagation_data( const fbtc::blockchain::transaction_id_type& transaction_id )
  {
    INVOKE_IN_IMPL(get_transaction_propagation_data, transaction_id);
  }

  message_propagation_data node::get_block_propagation_data( const fbtc::blockchain::block_id_type& block_id )
  {
