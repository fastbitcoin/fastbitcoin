...
#include <fbtc/db/level_pod_map.hpp>
  ...
  namespace detail
  {
    ...
    public:
      ...
      typedef boost::multi_index_container< potential_peer_database_entry, 
                                          ...
                                          > potential_peer_set;
    //private:
      typedef fbtc::db::level_pod_map<uint32_t, potential_peer_record> potential_peer_leveldb;
    ...
    void peer_database_impl::open(const fc::path& databaseFilename)
    {
      ...
      catch (const fbtc::db::level_pod_map_open_failure&) 
      {
