...
namespace fbtc { namespace net
  {
    ...
    void peer_connection::send_queueable_message(std::unique_ptr<queued_message>&& message_to_send)
    {
      ...
      if (_total_queued_messages_size > FBTC_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)
      {
        elog("send queue exceeded maximum size of ${max} bytes (current size ${current} bytes)",
             ("max", FBTC_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)("current", _total_queued_messages_size));
        try
        {
          close_connection();
        }
    ...
    {
      VERIFY_CORRECT_THREAD();
      fc::time_point_sec oldest_inventory_to_keep(fc::time_point::now() - fc::minutes(FBTC_NET_MAX_INVENTORY_SIZE_IN_MINUTES));
    ...
    {
      VERIFY_CORRECT_THREAD();
      return inventory_peer_advertised_to_us.size() > FBTC_NET_MAX_INVENTORY_SIZE_IN_MINUTES * FBTC_BLOCKCHAIN_MAX_TRX_PER_SECOND * 60;
    }
    ...
    bool peer_connection::is_inventory_advertised_to_us_list_full() const
    {
      ...
      // plus the maximum number of blocks that would be generated in FBTC_NET_MAX_INVENTORY_SIZE_IN_MINUTES (plus one,
      // to give us some wiggle room)
      return inventory_peer_advertised_to_us.size() >
        FBTC_NET_MAX_INVENTORY_SIZE_IN_MINUTES * FBTC_BLOCKCHAIN_MAX_TRX_PER_SECOND * 60 +
        (FBTC_NET_MAX_INVENTORY_SIZE_IN_MINUTES + 1) * 60 / FBTC_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
    }
