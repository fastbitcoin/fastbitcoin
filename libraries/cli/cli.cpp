...
namespace fbtc { namespace cli {
  ...
  FC_DECLARE_DERIVED_EXCEPTION( abort_cli_command, fbtc::cli::cli_exception, 11001, "command aborted by user" );
  FC_DECLARE_DERIVED_EXCEPTION( exit_cli_command, fbtc::cli::cli_exception, 11002, "exit CLI client requested by user" );
      ...
      )
  {
          if( input_stream == nullptr )
             FC_CAPTURE_AND_THROW( fbtc::cli::exit_cli_command ); //_input_stream != nullptr );
  ...
  namespace detail
  {
      class cli_impl
      {
         public:
            ...
            cli_impl(fbtc::client::client* client, std::istream* command_script, std::ostream* output_stream);
                                                       ...
                                                       fc::istream_ptr argument_stream )
            {
              ...
              catch( const fbtc::cli::abort_cli_command&)
              {
                throw;
              }
              catch( const fc::exception& e)
              ...
              if (command_is_valid)
              {
                ...
                catch( const fbtc::cli::exit_cli_command&)
                {
                  throw;
                }
                catch( const fc::exception& e )
            ...
            fc::variants parse_interactive_command(fc::buffered_istream& argument_stream, const string& command)
            {
              try
              {
                const fbtc::api::method_data& method_data = _rpc_server->get_method_data(command);
            ...
            { try {
              ...
              for( unsigned i = 0; i < method_data.parameters.size(); ++i )
              {
                ...
                {
                  if (method_data.parameters[i].classification != fbtc::api::required_positional)
                  {
                    return arguments;
                  }
                  else //if missing required argument, prompt for that argument
                  {
                    const fbtc::api::parameter_data& this_parameter = method_data.parameters[i];
                ...
                if (method_data.parameters[i].classification == fbtc::api::optional_named)
                  break;
              }
              return arguments;
            ...
            { try {
              const fbtc::api::parameter_data& this_parameter = method_data.parameters[parameter_index];
              if (this_parameter.type == "asset")
              {
                ...
                return fc::variant(fbtc::blockchain::asset(amount_as_int));
              }
              else if (this_parameter.type == "address")
              {
                ...
                if( ! fbtc::blockchain::address::is_valid(address_string) )
                {
                ...
                return fc::variant( fbtc::blockchain::address(address_string) );
              }
            ...
            fc::variant execute_interactive_command(const string& command, const fc::variants& arguments)
            {
              ...
              {
                _quit = true;
                FC_THROW_EXCEPTION( fbtc::cli::exit_cli_command, "quit command issued" );
              }
            ...
            { try {
                ...
                while( true )
                {
                    ...
                    if( input.empty() )
                      FC_THROW_EXCEPTION(fbtc::cli::abort_cli_command, "input aborted");
            ...
            void interactive_open_wallet()
            {
              ...
              *_out << "(q) Abort command\n";
              ...
              {
                FC_THROW_EXCEPTION(fbtc::cli::abort_cli_command, "");
              }
              else
              {
                  *_out << "Wrong answer!\n";
                                         ...
                                         const fc::variant& result)
            {
              ...
              {
                  string result_type;
                  const fbtc::api::method_data& method_data = _rpc_server->get_method_data(method_name);
                  result_type = method_data.return_type;

                  if (result_type == "asset")
                  {
                    *_out << (string)result.as<fbtc::blockchain::asset>() << "\n";
                  }
                  else if (result_type == "address")
                  {
                    *_out << (string)result.as<fbtc::blockchain::address>() << "\n";
                  }
...
#ifdef HAVE_READLINE
            typedef std::map<string, fbtc::api::method_data> method_data_map_type;
    ...
    cli_impl::cli_impl(fbtc::client::client* client, std::istream* command_script, std::ostream* output_stream)
    :_client(client)
    ...
    void cli_impl::initialize_method_data_if_necessary()
    {
      if (!_method_data_is_initialized)
      {
        ...
        vector<fbtc::api::method_data> method_data_list = _rpc_server->get_all_method_data();
    ...
    char** cli_impl::json_completion(const char* text, int start, int end)
    {
      ...
      else
      {
        ...
        if (!trimmed_command_to_parse.empty())
        {
          ...
          try
          {
            const fbtc::api::method_data& method_data = cli_impl_instance->_rpc_server->get_method_data(trimmed_command_to_parse);
            if (method_data.name == "help")
            {
                return rl_completion_matches(text, &json_command_completion_generator_function);
            }
          }
          catch( const fbtc::rpc::unknown_method& )
          {
            // do nothing
          }
        }
   ...
   cli::cli( fbtc::client::client* client, std::istream* command_script, std::ostream* output_stream)
